...
class VsalespersonsalesbyfiscalyearsdataViewStructure[Row](val prefix: Option[String], val extract: Row => VsalespersonsalesbyfiscalyearsdataViewRow, val merge: (Row, VsalespersonsalesbyfiscalyearsdataViewRow) => Row)
  ...
  override val salesterritory = new OptField[Name, Row](prefix, "salesterritory", None, Some(""""public"."Name""""))(x => extract(x).salesterritory, (row, value) => merge(row, extract(row).copy(salesterritory = value)))
  override val salestotal = new OptField[BigDecimal, Row](prefix, "salestotal", None, Some("numeric"))(x => extract(x).salestotal, (row, value) => merge(row, extract(row).copy(salestotal = value)))
  override val fiscalyear = new OptField[BigDecimal, Row](prefix, "fiscalyear", None, Some("numeric"))(x => extract(x).fiscalyear, (row, value) => merge(row, extract(row).copy(fiscalyear = value)))
