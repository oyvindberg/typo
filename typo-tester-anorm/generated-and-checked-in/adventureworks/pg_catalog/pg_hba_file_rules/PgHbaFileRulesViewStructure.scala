...
class PgHbaFileRulesViewStructure[Row](val prefix: Option[String], val extract: Row => PgHbaFileRulesViewRow, val merge: (Row, PgHbaFileRulesViewRow) => Row)
  ...
  override val lineNumber = new OptField[Int, Row](prefix, "line_number", None, Some("int4"))(x => extract(x).lineNumber, (row, value) => merge(row, extract(row).copy(lineNumber = value)))
  ...
  override val database = new OptField[Array[String], Row](prefix, "database", None, Some("_text"))(x => extract(x).database, (row, value) => merge(row, extract(row).copy(database = value)))
  override val userName = new OptField[Array[String], Row](prefix, "user_name", None, Some("_text"))(x => extract(x).userName, (row, value) => merge(row, extract(row).copy(userName = value)))
  ...
  override val options = new OptField[Array[String], Row](prefix, "options", None, Some("_text"))(x => extract(x).options, (row, value) => merge(row, extract(row).copy(options = value)))
