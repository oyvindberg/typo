...
class PgReplicationOriginStatusViewStructure[Row](val prefix: Option[String], val extract: Row => PgReplicationOriginStatusViewRow, val merge: (Row, PgReplicationOriginStatusViewRow) => Row)
  ...
  override val localId = new Field[/* oid */ Long, Row](prefix, "local_id", None, Some("oid"))(x => extract(x).localId, (row, value) => merge(row, extract(row).copy(localId = value)))
  override val externalId = new Field[String, Row](prefix, "external_id", None, None)(x => extract(x).externalId, (row, value) => merge(row, extract(row).copy(externalId = value)))
  override val remoteLsn = new Field[/* pg_lsn */ Long, Row](prefix, "remote_lsn", None, Some("pg_lsn"))(x => extract(x).remoteLsn, (row, value) => merge(row, extract(row).copy(remoteLsn = value)))
  override val localLsn = new Field[/* pg_lsn */ Long, Row](prefix, "local_lsn", None, Some("pg_lsn"))(x => extract(x).localLsn, (row, value) => merge(row, extract(row).copy(localLsn = value)))
