...
class PgStatDatabaseConflictsViewStructure[Row](val prefix: Option[String], val extract: Row => PgStatDatabaseConflictsViewRow, val merge: (Row, PgStatDatabaseConflictsViewRow) => Row)
  ...
  override val conflTablespace = new OptField[Long, Row](prefix, "confl_tablespace", None, Some("int8"))(x => extract(x).conflTablespace, (row, value) => merge(row, extract(row).copy(conflTablespace = value)))
  override val conflLock = new OptField[Long, Row](prefix, "confl_lock", None, Some("int8"))(x => extract(x).conflLock, (row, value) => merge(row, extract(row).copy(conflLock = value)))
  override val conflSnapshot = new OptField[Long, Row](prefix, "confl_snapshot", None, Some("int8"))(x => extract(x).conflSnapshot, (row, value) => merge(row, extract(row).copy(conflSnapshot = value)))
  override val conflBufferpin = new OptField[Long, Row](prefix, "confl_bufferpin", None, Some("int8"))(x => extract(x).conflBufferpin, (row, value) => merge(row, extract(row).copy(conflBufferpin = value)))
  override val conflDeadlock = new OptField[Long, Row](prefix, "confl_deadlock", None, Some("int8"))(x => extract(x).conflDeadlock, (row, value) => merge(row, extract(row).copy(conflDeadlock = value)))
