...
class PgStatProgressClusterViewStructure[Row](val prefix: Option[String], val extract: Row => PgStatProgressClusterViewRow, val merge: (Row, PgStatProgressClusterViewRow) => Row)
  ...
  override val clusterIndexRelid = new Field[/* oid */ Long, Row](prefix, "cluster_index_relid", None, Some("oid"))(x => extract(x).clusterIndexRelid, (row, value) => merge(row, extract(row).copy(clusterIndexRelid = value)))
  override val heapTuplesScanned = new Field[Long, Row](prefix, "heap_tuples_scanned", None, Some("int8"))(x => extract(x).heapTuplesScanned, (row, value) => merge(row, extract(row).copy(heapTuplesScanned = value)))
  override val heapTuplesWritten = new Field[Long, Row](prefix, "heap_tuples_written", None, Some("int8"))(x => extract(x).heapTuplesWritten, (row, value) => merge(row, extract(row).copy(heapTuplesWritten = value)))
  ...
  override val indexRebuildCount = new Field[Long, Row](prefix, "index_rebuild_count", None, Some("int8"))(x => extract(x).indexRebuildCount, (row, value) => merge(row, extract(row).copy(indexRebuildCount = value)))
