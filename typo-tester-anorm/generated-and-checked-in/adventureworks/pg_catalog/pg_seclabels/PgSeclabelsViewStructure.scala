...
class PgSeclabelsViewStructure[Row](val prefix: Option[String], val extract: Row => PgSeclabelsViewRow, val merge: (Row, PgSeclabelsViewRow) => Row)
  ...
  override val objoid = new OptField[/* oid */ Long, Row](prefix, "objoid", None, Some("oid"))(x => extract(x).objoid, (row, value) => merge(row, extract(row).copy(objoid = value)))
  override val classoid = new OptField[/* oid */ Long, Row](prefix, "classoid", None, Some("oid"))(x => extract(x).classoid, (row, value) => merge(row, extract(row).copy(classoid = value)))
  override val objsubid = new OptField[Int, Row](prefix, "objsubid", None, Some("int4"))(x => extract(x).objsubid, (row, value) => merge(row, extract(row).copy(objsubid = value)))
  ...
  override val objnamespace = new OptField[/* oid */ Long, Row](prefix, "objnamespace", None, Some("oid"))(x => extract(x).objnamespace, (row, value) => merge(row, extract(row).copy(objnamespace = value)))
