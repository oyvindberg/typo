...
class TransformsViewStructure[Row](val prefix: Option[String], val extract: Row => TransformsViewRow, val merge: (Row, TransformsViewRow) => Row)
  ...
  override val groupName = new Field[SqlIdentifier, Row](prefix, "group_name", None, Some("name"))(x => extract(x).groupName, (row, value) => merge(row, extract(row).copy(groupName = value)))
  override val transformType = new Field[CharacterData, Row](prefix, "transform_type", None, Some("varchar"))(x => extract(x).transformType, (row, value) => merge(row, extract(row).copy(transformType = value)))
