...
class PasswordRepoImpl extends PasswordRepo {
  ...
  def deleteById(businessentityid: BusinessentityId)(implicit c: Connection): Boolean = SQL"""delete from person.password where "businessentityid" = ${ParameterValue(businessentityid, null, BusinessentityId.toStatement)}""".executeUpdate() > 0
  ...
  def insertStreaming(unsaved: Iterator[PasswordRow], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY person.password("businessentityid", "passwordhash", "passwordsalt", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(PasswordRow.text, c)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Iterator[PasswordRowUnsaved], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY person.password("businessentityid", "passwordhash", "passwordsalt", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(PasswordRowUnsaved.text, c)
  def select: SelectBuilder[PasswordFields, PasswordRow] = SelectBuilderSql("person.password", PasswordFields.structure, PasswordRow.rowParser)
  ...
  def update: UpdateBuilder[PasswordFields, PasswordRow] = UpdateBuilder("person.password", PasswordFields.structure, PasswordRow.rowParser)
