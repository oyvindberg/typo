...
class EmailaddressRepoImpl extends EmailaddressRepo {
  ...
  def insertStreaming(unsaved: Iterator[EmailaddressRow], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY person.emailaddress("businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(EmailaddressRow.text, c)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Iterator[EmailaddressRowUnsaved], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY person.emailaddress("businessentityid", "emailaddress", "emailaddressid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(EmailaddressRowUnsaved.text, c)
  def select: SelectBuilder[EmailaddressFields, EmailaddressRow] = SelectBuilderSql("person.emailaddress", EmailaddressFields.structure, EmailaddressRow.rowParser)
  ...
  def update: UpdateBuilder[EmailaddressFields, EmailaddressRow] = UpdateBuilder("person.emailaddress", EmailaddressFields.structure, EmailaddressRow.rowParser)
  ...
  def upsertBatch(unsaved: Iterable[EmailaddressRow])(implicit c: Connection): List[EmailaddressRow] = {
    def toNamedParameter(row: EmailaddressRow): List[NamedParameter] = List(
      ...
      NamedParameter("emailaddress", ParameterValue(row.emailaddress, null, ToStatement.optionToStatement(ToStatement.stringToStatement, ParameterMetaData.StringParameterMetaData))),
    ...
    unsaved.toList match {
      ...
      case head :: rest =>
        ...
        ).executeReturning(EmailaddressRow.rowParser(1).*)
    }
  }
  ...
  def upsertStreaming(unsaved: Iterator[EmailaddressRow], batchSize: Int = 10000)(implicit c: Connection): Int = {
    ...
    SQL"""insert into person.emailaddress("businessentityid", "emailaddressid", "emailaddress", "rowguid", "modifieddate")
          ...
          do update set
            ...
            "modifieddate" = EXCLUDED."modifieddate"
          ;
          drop table emailaddress_TEMP;""".executeUpdate()
  }
}