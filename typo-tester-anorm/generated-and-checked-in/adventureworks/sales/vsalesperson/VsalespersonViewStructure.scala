...
class VsalespersonViewStructure[Row](val prefix: Option[String], val extract: Row => VsalespersonViewRow, val merge: (Row, VsalespersonViewRow) => Row)
  ...
  override val territoryname = new OptField[Name, Row](prefix, "territoryname", None, Some("varchar"))(x => extract(x).territoryname, (row, value) => merge(row, extract(row).copy(territoryname = value)))
