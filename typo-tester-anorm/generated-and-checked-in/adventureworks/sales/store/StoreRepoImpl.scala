...
class StoreRepoImpl extends StoreRepo {
  ...
  def deleteById(businessentityid: BusinessentityId)(implicit c: Connection): Boolean = SQL"""delete from sales.store where "businessentityid" = ${ParameterValue(businessentityid, null, BusinessentityId.toStatement)}""".executeUpdate() > 0
  ...
  def insertStreaming(unsaved: Iterator[StoreRow], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY sales.store("businessentityid", "name", "salespersonid", "demographics", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(StoreRow.text, c)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Iterator[StoreRowUnsaved], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY sales.store("businessentityid", "name", "salespersonid", "demographics", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(StoreRowUnsaved.text, c)
  def select: SelectBuilder[StoreFields, StoreRow] = SelectBuilderSql("sales.store", StoreFields.structure, StoreRow.rowParser)
  ...
  def update: UpdateBuilder[StoreFields, StoreRow] = UpdateBuilder("sales.store", StoreFields.structure, StoreRow.rowParser)
