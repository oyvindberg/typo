...
class TestInsert(val random: Random, val domainInsert: TestDomainInsert) {
  ...
  )(implicit c: Connection): IdentityTestRow = (new IdentityTestRepoImpl).insert(new IdentityTestRowUnsaved(name = name, alwaysGenerated = alwaysGenerated, defaultGenerated = defaultGenerated))
  def publicPgtest(
    box: TypoBox,
    bytea: TypoBytea,
    circle: TypoCircle,
