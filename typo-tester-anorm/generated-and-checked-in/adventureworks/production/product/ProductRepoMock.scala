...
class ProductRepoMock(val toRow: Function1[ProductRowUnsaved, ProductRow], val map: scala.collection.mutable.Map[ProductId, ProductRow] = scala.collection.mutable.Map.empty) extends ProductRepo {
  ...
  def deleteById(productid: ProductId)(implicit c: Connection): Boolean = map.remove(productid).isDefined
  def deleteByIds(productids: Array[ProductId])(implicit c: Connection): Int = productids.map(id => map.remove(id)).count(_.isDefined)
  ...
  def insert(unsaved: ProductRowUnsaved)(implicit c: Connection): ProductRow = insert(toRow(unsaved))
  ...
  def selectAll(implicit c: Connection): List[ProductRow] = map.values.toList
  def selectById(productid: ProductId)(implicit c: Connection): Option[ProductRow] = map.get(productid)
  def selectByIds(productids: Array[ProductId])(implicit c: Connection): List[ProductRow] = productids.flatMap(map.get).toList
