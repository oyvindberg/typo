...
class PgCursorsViewStructure[Row](val prefix: Option[String], val extract: Row => PgCursorsViewRow, val merge: (Row, PgCursorsViewRow) => Row)
  ...
  override val creationTime = new OptField[TypoOffsetDateTime, Row](prefix, "creation_time", Some("text"), Some("timestamptz"))(x => extract(x).creationTime, (row, value) => merge(row, extract(row).copy(creationTime = value)))
