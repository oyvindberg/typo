...
productionproduct0 as (
  (select productionproduct0 from "production"."product" productionproduct0 WHERE ((productionproduct0).class = ? )  AND (((productionproduct0).daystomanufacture > ? )  OR ((productionproduct0).daystomanufacture <= ? ) )  AND ((productionproduct0).productline = ? ) ) 
) ,
productionunitmeasure0 as (
  (select productionunitmeasure0 from "production"."unitmeasure" productionunitmeasure0 WHERE ((productionunitmeasure0).name LIKE ? ) ) 
) ,
join_cte0 as (
