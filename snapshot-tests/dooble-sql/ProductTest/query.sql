with 
product0 as (
  (select product0 from production.product product0 WHERE ((product0).class = ? )  AND (((product0).daystomanufacture > ? )  OR ((product0).daystomanufacture <= ? ) )  AND ((product0).productline = ? ) ) 
) ,
unitmeasure0 as (
  (select unitmeasure0 from production.unitmeasure unitmeasure0 WHERE ((unitmeasure0).name LIKE ? ) ) 
) ,
join_cte0 as (
