with 
product0 as (
  (select product0 from production.product product0 WHERE NOT ((product0).name LIKE ? )   AND NOT (((product0).name || (product0).color)  LIKE ? )   AND ((product0).daystomanufacture > ? )  AND ((product0).modifieddate < ?::timestamp ) ) 
) ,
productmodel0 as (
