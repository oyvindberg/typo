...
join (
  select "productmodelid","name","catalogdescription","instructions","rowguid","modifieddate"::text from production.productmodel productmodel0
   where (length(productmodel0.name) > ?::INTEGER)
) productmodel0 on (product0.productmodelid = productmodel0.productmodelid)
left join (
  select "productmodelid","name","catalogdescription","instructions","rowguid","modifieddate"::text from production.productmodel productmodel1
   where (length(productmodel1.name) > ?::INTEGER)
) productmodel1 on ((product0.productmodelid = productmodel1.productmodelid) AND ?::BOOLEAN)
