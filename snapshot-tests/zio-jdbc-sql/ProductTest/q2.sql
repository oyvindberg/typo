with
product0 as (
  (select product0 from production.product product0  WHERE (product0).productid = ANY(?) AND (length((product0).name) > ?::int4) AND NOT (((product0).name || (product0).color) LIKE ?::text) AND (coalesce(length((product0).color), ?::int4) > ?::int4) AND ((product0).modifieddate < ?::timestamp))
),
productmodel0 as (
  (select productmodel0 from production.productmodel productmodel0  WHERE NOT ((productmodel0).name LIKE ?::text))
),
join_cte0 as (
  ...
  on ((product0).productmodelid = (productmodel0).productmodelid)
   WHERE ((productmodel0).productmodelid > ?::int4)
),
productmodel1 as (
  (select productmodel1 from production.productmodel productmodel1  WHERE NOT ((productmodel1).name LIKE ?::text))
),
left_join_cte0 as (
