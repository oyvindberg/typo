with
product0 as (
  (select product0 from production.product product0  WHERE NOT ((product0).name LIKE ?::text) AND NOT (((product0).name || (product0).color) LIKE ?::text) AND ((product0).daystomanufacture > ?::int4) AND ((product0).modifieddate < ?::timestamp))
),
productmodel0 as (
