...
productionproduct0 as (
  (select productionproduct0 from "production"."product" productionproduct0  WHERE NOT ((productionproduct0).name  LIKE ?::text) AND NOT (((productionproduct0).name  || (productionproduct0).color ) LIKE ?::text) AND ((productionproduct0).daystomanufacture  > ?::int4) AND ((productionproduct0).modifieddate  < ?::timestamp))
),
