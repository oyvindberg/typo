...
class TablesViewStructure[Row](val prefix: Option[String], val extract: Row => TablesViewRow, val merge: (Row, TablesViewRow) => Row)
  ...
  override val tableType = new OptField[CharacterData, Row](prefix, "table_type", None, Some("information_schema.character_data"))(x => extract(x).tableType, (row, value) => merge(row, extract(row).copy(tableType = value)))
  override val selfReferencingColumnName = new OptField[SqlIdentifier, Row](prefix, "self_referencing_column_name", None, Some("information_schema.sql_identifier"))(x => extract(x).selfReferencingColumnName, (row, value) => merge(row, extract(row).copy(selfReferencingColumnName = value)))
  override val referenceGeneration = new OptField[CharacterData, Row](prefix, "reference_generation", None, Some("information_schema.character_data"))(x => extract(x).referenceGeneration, (row, value) => merge(row, extract(row).copy(referenceGeneration = value)))
  ...
  override val isTyped = new OptField[YesOrNo, Row](prefix, "is_typed", None, Some("information_schema.yes_or_no"))(x => extract(x).isTyped, (row, value) => merge(row, extract(row).copy(isTyped = value)))
  override val commitAction = new OptField[CharacterData, Row](prefix, "commit_action", None, Some("information_schema.character_data"))(x => extract(x).commitAction, (row, value) => merge(row, extract(row).copy(commitAction = value)))
