...
object LocationRepoImpl extends LocationRepo {
  ...
  override def insert(unsaved: LocationRowUnsaved): ConnectionIO[LocationRow] = {
    val fs = List(
      ...
      unsaved.locationid match {
        ...
        case Defaulted.Provided(value) => Some((Fragment.const(s"locationid"), fr"locationid = ${value: LocationId}::int4"))
      },
      unsaved.costrate match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((Fragment.const(s"costrate"), fr"costrate = ${value: BigDecimal}::numeric"))
      },
      unsaved.availability match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((Fragment.const(s"availability"), fr"availability = ${value: BigDecimal}::numeric"))
      },
  ...
  override def selectByIds(locationids: Array[LocationId]): Stream[ConnectionIO, LocationRow] = {
    sql"""select locationid, "name", costrate, availability, modifieddate from production.location where locationid in $locationids""".query[LocationRow].stream
  }
