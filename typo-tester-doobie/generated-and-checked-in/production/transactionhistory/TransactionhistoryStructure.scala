...
class TransactionhistoryStructure[Row](val prefix: Option[String], val extract: Row => TransactionhistoryRow, val merge: (Row, TransactionhistoryRow) => Row)
  extends Relation[TransactionhistoryFields, TransactionhistoryRow, Row]
    with TransactionhistoryFields[Row] { outer =>
  ...
  override val transactiontype = new Field[/* bpchar */ String, Row](prefix, "transactiontype", None, Some("bpchar"))(x => extract(x).transactiontype, (row, value) => merge(row, extract(row).copy(transactiontype = value)))
  ...
  override def copy[NewRow](prefix: Option[String], extract: NewRow => TransactionhistoryRow, merge: (NewRow, TransactionhistoryRow) => NewRow): TransactionhistoryStructure[NewRow] =
    new TransactionhistoryStructure(prefix, extract, merge)
}
