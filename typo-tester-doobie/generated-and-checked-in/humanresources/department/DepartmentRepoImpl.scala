...
object DepartmentRepoImpl extends DepartmentRepo {
  ...
  override def insert(unsaved: DepartmentRowUnsaved): ConnectionIO[DepartmentRow] = {
    val fs = List(
      ...
      Some((Fragment.const(s"groupname"), fr"groupname = ${unsaved.groupname}::public.Name")),
      unsaved.departmentid match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((Fragment.const(s"departmentid"), fr"departmentid = ${value: DepartmentId}::int4"))
      },
  ...
  override def selectByIds(departmentids: Array[DepartmentId]): Stream[ConnectionIO, DepartmentRow] = {
    sql"""select departmentid, "name", groupname, modifieddate from humanresources.department where departmentid in $departmentids""".query[DepartmentRow].stream
  }
