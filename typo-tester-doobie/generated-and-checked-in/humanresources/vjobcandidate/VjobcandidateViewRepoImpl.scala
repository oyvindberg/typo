  ...
  implicit val read: Read[VjobcandidateViewRow] =
    new Read[VjobcandidateViewRow](
      ...
      unsafeGet = (rs: ResultSet, i: Int) => VjobcandidateViewRow(
        ...
        `Name.Prefix` = Get[String].unsafeGetNullable(rs, i + 2),
        `Name.First` = Get[String].unsafeGetNullable(rs, i + 3),
        `Name.Middle` = Get[String].unsafeGetNullable(rs, i + 4),
        `Name.Last` = Get[String].unsafeGetNullable(rs, i + 5),
        `Name.Suffix` = Get[String].unsafeGetNullable(rs, i + 6),
        Skills = Get[String].unsafeGetNullable(rs, i + 7),
        `Addr.Type` = Get[String].unsafeGetNullable(rs, i + 8),
        `Addr.Loc.CountryRegion` = Get[String].unsafeGetNullable(rs, i + 9),
        `Addr.Loc.State` = Get[String].unsafeGetNullable(rs, i + 10),
        `Addr.Loc.City` = Get[String].unsafeGetNullable(rs, i + 11),
        `Addr.PostalCode` = Get[String].unsafeGetNullable(rs, i + 12),
