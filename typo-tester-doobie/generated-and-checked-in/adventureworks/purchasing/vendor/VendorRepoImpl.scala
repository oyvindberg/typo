...
class VendorRepoImpl extends VendorRepo {
  ...
  override def deleteById(businessentityid: BusinessentityId): ConnectionIO[Boolean] = {
    sql"""delete from "purchasing"."vendor" where "businessentityid" = ${fromWrite(businessentityid)(new Write.Single(BusinessentityId.put))}""".update.run.map(_ > 0)
  }
  ...
  override def insert(unsaved: VendorRowUnsaved): ConnectionIO[VendorRow] = {
    val fs = List(
      ...
      Some((Fragment.const0(s""""creditrating""""), fr"${fromWrite(unsaved.creditrating)(new Write.Single(TypoShort.put))}::int2")),
      Some((Fragment.const0(s""""purchasingwebserviceurl""""), fr"${fromWrite(unsaved.purchasingwebserviceurl)(new Write.SingleOpt(Meta.StringMeta.put))}")),
      unsaved.preferredvendorstatus match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""preferredvendorstatus""""), fr"${fromWrite(value: Flag)(new Write.Single(Flag.put))}::bool"))
      },
      unsaved.activeflag match {
        case Defaulted.UseDefault => None
        case Defaulted.Provided(value) => Some((Fragment.const0(s""""activeflag""""), fr"${fromWrite(value: Flag)(new Write.Single(Flag.put))}::bool"))
      },
  ...
  override def selectById(businessentityid: BusinessentityId): ConnectionIO[Option[VendorRow]] = {
    sql"""select "businessentityid", "accountnumber", "name", "creditrating", "preferredvendorstatus", "activeflag", "purchasingwebserviceurl", "modifieddate"::text from "purchasing"."vendor" where "businessentityid" = ${fromWrite(businessentityid)(new Write.Single(BusinessentityId.put))}""".query(using VendorRow.read).option
  }
  ...
  override def update(row: VendorRow): ConnectionIO[Option[VendorRow]] = {
    ...
    sql"""update "purchasing"."vendor"
          set "accountnumber" = ${fromWrite(row.accountnumber)(new Write.Single(AccountNumber.put))}::varchar,
              ...
              "creditrating" = ${fromWrite(row.creditrating)(new Write.Single(TypoShort.put))}::int2,
              "preferredvendorstatus" = ${fromWrite(row.preferredvendorstatus)(new Write.Single(Flag.put))}::bool,
  ...
  override def upsert(unsaved: VendorRow): ConnectionIO[VendorRow] = {
    sql"""insert into "purchasing"."vendor"("businessentityid", "accountnumber", "name", "creditrating", "preferredvendorstatus", "activeflag", "purchasingwebserviceurl", "modifieddate")
          values (
            ...
            ${fromWrite(unsaved.accountnumber)(new Write.Single(AccountNumber.put))}::varchar,
            ${fromWrite(unsaved.name)(new Write.Single(Name.put))}::varchar,
            ${fromWrite(unsaved.creditrating)(new Write.Single(TypoShort.put))}::int2,
            ${fromWrite(unsaved.preferredvendorstatus)(new Write.Single(Flag.put))}::bool,
            ${fromWrite(unsaved.activeflag)(new Write.Single(Flag.put))}::bool,
            ${fromWrite(unsaved.purchasingwebserviceurl)(new Write.SingleOpt(Meta.StringMeta.put))},
