...
class KeyColumnUsageViewStructure[Row](val prefix: Option[String], val extract: Row => KeyColumnUsageViewRow, val merge: (Row, KeyColumnUsageViewRow) => Row)
  ...
  override val positionInUniqueConstraint = new Field[CardinalNumber, Row](prefix, "position_in_unique_constraint", None, Some("information_schema.cardinal_number"))(x => extract(x).positionInUniqueConstraint, (row, value) => merge(row, extract(row).copy(positionInUniqueConstraint = value)))
