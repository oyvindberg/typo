...
class SalesterritoryRepoImpl extends SalesterritoryRepo {
  ...
  def deleteById(territoryid: SalesterritoryId): ConnectionIO[Boolean] = sql"""delete from sales.salesterritory where "territoryid" = ${fromWrite(territoryid)(Write.fromPut(SalesterritoryId.put))}""".update.run.map(_ > 0)
  def deleteByIds(territoryids: Array[SalesterritoryId]): ConnectionIO[Int] = sql"""delete from sales.salesterritory where "territoryid" = ANY(${fromWrite(territoryids)(Write.fromPut(SalesterritoryId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, SalesterritoryRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY sales.salesterritory("territoryid", "name", "countryregioncode", "group", "salesytd", "saleslastyear", "costytd", "costlastyear", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using SalesterritoryRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, SalesterritoryRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY sales.salesterritory("name", "countryregioncode", "group", "territoryid", "salesytd", "saleslastyear", "costytd", "costlastyear", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using SalesterritoryRowUnsaved.text)
  def select: SelectBuilder[SalesterritoryFields, SalesterritoryRow] = SelectBuilderSql("sales.salesterritory", SalesterritoryFields.structure, SalesterritoryRow.read)
  ...
  def update: UpdateBuilder[SalesterritoryFields, SalesterritoryRow] = UpdateBuilder("sales.salesterritory", SalesterritoryFields.structure, SalesterritoryRow.read)
