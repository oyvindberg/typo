...
class ProductphotoRepoImpl extends ProductphotoRepo {
  ...
  def deleteById(productphotoid: ProductphotoId): ConnectionIO[Boolean] = sql"""delete from production.productphoto where "productphotoid" = ${fromWrite(productphotoid)(Write.fromPut(ProductphotoId.put))}""".update.run.map(_ > 0)
  def deleteByIds(productphotoids: Array[ProductphotoId]): ConnectionIO[Int] = sql"""delete from production.productphoto where "productphotoid" = ANY(${fromWrite(productphotoids)(Write.fromPut(ProductphotoId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, ProductphotoRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.productphoto("productphotoid", "thumbnailphoto", "thumbnailphotofilename", "largephoto", "largephotofilename", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using ProductphotoRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, ProductphotoRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.productphoto("thumbnailphoto", "thumbnailphotofilename", "largephoto", "largephotofilename", "productphotoid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using ProductphotoRowUnsaved.text)
  def select: SelectBuilder[ProductphotoFields, ProductphotoRow] = SelectBuilderSql("production.productphoto", ProductphotoFields.structure, ProductphotoRow.read)
  ...
  def update: UpdateBuilder[ProductphotoFields, ProductphotoRow] = UpdateBuilder("production.productphoto", ProductphotoFields.structure, ProductphotoRow.read)
