...
class ProductproductphotoRepoImpl extends ProductproductphotoRepo {
  ...
  def deleteById(compositeId: ProductproductphotoId): ConnectionIO[Boolean] = sql"""delete from production.productproductphoto where "productid" = ${fromWrite(compositeId.productid)(Write.fromPut(ProductId.put))} AND "productphotoid" = ${fromWrite(compositeId.productphotoid)(Write.fromPut(ProductphotoId.put))}""".update.run.map(_ > 0)
  def deleteByIds(compositeIds: Array[ProductproductphotoId]): ConnectionIO[Int] = {
    ...
    sql"""delete
          ...
          in (select unnest(${fromWrite(productid)(Write.fromPut(ProductId.arrayPut))}), unnest(${fromWrite(productphotoid)(Write.fromPut(ProductphotoId.arrayPut))}))
       """.update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, ProductproductphotoRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.productproductphoto("productid", "productphotoid", "primary", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using ProductproductphotoRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, ProductproductphotoRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.productproductphoto("productid", "productphotoid", "primary", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using ProductproductphotoRowUnsaved.text)
  def select: SelectBuilder[ProductproductphotoFields, ProductproductphotoRow] = SelectBuilderSql("production.productproductphoto", ProductproductphotoFields.structure, ProductproductphotoRow.read)
  ...
  def update: UpdateBuilder[ProductproductphotoFields, ProductproductphotoRow] = UpdateBuilder("production.productproductphoto", ProductproductphotoFields.structure, ProductproductphotoRow.read)
