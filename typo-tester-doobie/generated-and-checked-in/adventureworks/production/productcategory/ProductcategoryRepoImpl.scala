...
class ProductcategoryRepoImpl extends ProductcategoryRepo {
  ...
  def deleteById(productcategoryid: ProductcategoryId): ConnectionIO[Boolean] = sql"""delete from production.productcategory where "productcategoryid" = ${fromWrite(productcategoryid)(Write.fromPut(ProductcategoryId.put))}""".update.run.map(_ > 0)
  def deleteByIds(productcategoryids: Array[ProductcategoryId]): ConnectionIO[Int] = sql"""delete from production.productcategory where "productcategoryid" = ANY(${fromWrite(productcategoryids)(Write.fromPut(ProductcategoryId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, ProductcategoryRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.productcategory("productcategoryid", "name", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using ProductcategoryRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, ProductcategoryRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.productcategory("name", "productcategoryid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using ProductcategoryRowUnsaved.text)
  def select: SelectBuilder[ProductcategoryFields, ProductcategoryRow] = SelectBuilderSql("production.productcategory", ProductcategoryFields.structure, ProductcategoryRow.read)
  def selectAll: Stream[ConnectionIO, ProductcategoryRow] = sql"""select "productcategoryid", "name", "rowguid", "modifieddate"::text from production.productcategory""".query(using ProductcategoryRow.read).stream
  def selectById(productcategoryid: ProductcategoryId): ConnectionIO[Option[ProductcategoryRow]] = sql"""select "productcategoryid", "name", "rowguid", "modifieddate"::text from production.productcategory where "productcategoryid" = ${fromWrite(productcategoryid)(Write.fromPut(ProductcategoryId.put))}""".query(using ProductcategoryRow.read).option
  def selectByIds(productcategoryids: Array[ProductcategoryId]): Stream[ConnectionIO, ProductcategoryRow] = sql"""select "productcategoryid", "name", "rowguid", "modifieddate"::text from production.productcategory where "productcategoryid" = ANY(${fromWrite(productcategoryids)(Write.fromPut(ProductcategoryId.arrayPut))})""".query(using ProductcategoryRow.read).stream
  ...
  def update: UpdateBuilder[ProductcategoryFields, ProductcategoryRow] = UpdateBuilder("production.productcategory", ProductcategoryFields.structure, ProductcategoryRow.read)
