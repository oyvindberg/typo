...
class ScrapreasonRepoImpl extends ScrapreasonRepo {
  ...
  def deleteById(scrapreasonid: ScrapreasonId): ConnectionIO[Boolean] = sql"""delete from production.scrapreason where "scrapreasonid" = ${fromWrite(scrapreasonid)(Write.fromPut(ScrapreasonId.put))}""".update.run.map(_ > 0)
  def deleteByIds(scrapreasonids: Array[ScrapreasonId]): ConnectionIO[Int] = sql"""delete from production.scrapreason where "scrapreasonid" = ANY(${fromWrite(scrapreasonids)(Write.fromPut(ScrapreasonId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, ScrapreasonRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.scrapreason("scrapreasonid", "name", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using ScrapreasonRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, ScrapreasonRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.scrapreason("name", "scrapreasonid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using ScrapreasonRowUnsaved.text)
  def select: SelectBuilder[ScrapreasonFields, ScrapreasonRow] = SelectBuilderSql("production.scrapreason", ScrapreasonFields.structure, ScrapreasonRow.read)
  ...
  def selectById(scrapreasonid: ScrapreasonId): ConnectionIO[Option[ScrapreasonRow]] = sql"""select "scrapreasonid", "name", "modifieddate"::text from production.scrapreason where "scrapreasonid" = ${fromWrite(scrapreasonid)(Write.fromPut(ScrapreasonId.put))}""".query(using ScrapreasonRow.read).option
  def selectByIds(scrapreasonids: Array[ScrapreasonId]): Stream[ConnectionIO, ScrapreasonRow] = sql"""select "scrapreasonid", "name", "modifieddate"::text from production.scrapreason where "scrapreasonid" = ANY(${fromWrite(scrapreasonids)(Write.fromPut(ScrapreasonId.arrayPut))})""".query(using ScrapreasonRow.read).stream
  ...
  def update: UpdateBuilder[ScrapreasonFields, ScrapreasonRow] = UpdateBuilder("production.scrapreason", ScrapreasonFields.structure, ScrapreasonRow.read)
