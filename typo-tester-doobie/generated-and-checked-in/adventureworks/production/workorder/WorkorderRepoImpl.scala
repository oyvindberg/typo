...
class WorkorderRepoImpl extends WorkorderRepo {
  def delete: DeleteBuilder[WorkorderFields, WorkorderRow] = DeleteBuilder("production.workorder", WorkorderFields.structure)
  def deleteById(workorderid: WorkorderId): ConnectionIO[Boolean] = sql"""delete from production.workorder where "workorderid" = ${fromWrite(workorderid)(Write.fromPut(WorkorderId.put))}""".update.run.map(_ > 0)
  def deleteByIds(workorderids: Array[WorkorderId]): ConnectionIO[Int] = sql"""delete from production.workorder where "workorderid" = ANY(${fromWrite(workorderids)(Write.fromPut(WorkorderId.arrayPut))})""".update.run
  ...
  def insert(unsaved: WorkorderRowUnsaved): ConnectionIO[WorkorderRow] = {
    val fs = List(
      Some((Fragment.const0(s""""productid""""), fr"${fromWrite(unsaved.productid)(Write.fromPut(ProductId.put))}::int4")),
                      Some((Fragment.const0(s""""orderqty""""), fr"${fromWrite(unsaved.orderqty)(Write.fromPut(Meta.IntMeta.put))}::int4")),
                      Some((Fragment.const0(s""""scrappedqty""""), fr"${fromWrite(unsaved.scrappedqty)(Write.fromPut(TypoShort.put))}::int2")),
                      ...
                      Some((Fragment.const0(s""""scrapreasonid""""), fr"${fromWrite(unsaved.scrapreasonid)(Write.fromPutOption(ScrapreasonId.put))}::int2")),
    unsaved.workorderid match {
      case Defaulted.UseDefault() => None
      case Defaulted.Provided(value) => Some((Fragment.const0(s""""workorderid""""), fr"${fromWrite(value: WorkorderId)(Write.fromPut(WorkorderId.put))}::int4"))
    },
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, WorkorderRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.workorder("workorderid", "productid", "orderqty", "scrappedqty", "startdate", "enddate", "duedate", "scrapreasonid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using WorkorderRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, WorkorderRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.workorder("productid", "orderqty", "scrappedqty", "startdate", "enddate", "duedate", "scrapreasonid", "workorderid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using WorkorderRowUnsaved.text)
  def select: SelectBuilder[WorkorderFields, WorkorderRow] = SelectBuilderSql("production.workorder", WorkorderFields.structure, WorkorderRow.read)
  def selectAll: Stream[ConnectionIO, WorkorderRow] = sql"""select "workorderid", "productid", "orderqty", "scrappedqty", "startdate"::text, "enddate"::text, "duedate"::text, "scrapreasonid", "modifieddate"::text from production.workorder""".query(using WorkorderRow.read).stream
  def selectById(workorderid: WorkorderId): ConnectionIO[Option[WorkorderRow]] = sql"""select "workorderid", "productid", "orderqty", "scrappedqty", "startdate"::text, "enddate"::text, "duedate"::text, "scrapreasonid", "modifieddate"::text from production.workorder where "workorderid" = ${fromWrite(workorderid)(Write.fromPut(WorkorderId.put))}""".query(using WorkorderRow.read).option
  def selectByIds(workorderids: Array[WorkorderId]): Stream[ConnectionIO, WorkorderRow] = sql"""select "workorderid", "productid", "orderqty", "scrappedqty", "startdate"::text, "enddate"::text, "duedate"::text, "scrapreasonid", "modifieddate"::text from production.workorder where "workorderid" = ANY(${fromWrite(workorderids)(Write.fromPut(WorkorderId.arrayPut))})""".query(using WorkorderRow.read).stream
  ...
  def update: UpdateBuilder[WorkorderFields, WorkorderRow] = UpdateBuilder("production.workorder", WorkorderFields.structure, WorkorderRow.read)
