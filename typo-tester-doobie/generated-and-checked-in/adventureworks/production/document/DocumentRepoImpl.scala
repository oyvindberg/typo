...
class DocumentRepoImpl extends DocumentRepo {
  def delete: DeleteBuilder[DocumentFields, DocumentRow] = DeleteBuilder("production.document", DocumentFields.structure)
  def deleteById(documentnode: DocumentId): ConnectionIO[Boolean] = sql"""delete from production.document where "documentnode" = ${fromWrite(documentnode)(Write.fromPut(DocumentId.put))}""".update.run.map(_ > 0)
  def deleteByIds(documentnodes: Array[DocumentId]): ConnectionIO[Int] = sql"""delete from production.document where "documentnode" = ANY(${fromWrite(documentnodes)(Write.fromPut(DocumentId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, DocumentRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.document("title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate", "documentnode") FROM STDIN""").copyIn(unsaved, batchSize)(using DocumentRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, DocumentRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY production.document("title", "owner", "filename", "fileextension", "revision", "status", "documentsummary", "document", "folderflag", "changenumber", "rowguid", "modifieddate", "documentnode") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using DocumentRowUnsaved.text)
  def select: SelectBuilder[DocumentFields, DocumentRow] = SelectBuilderSql("production.document", DocumentFields.structure, DocumentRow.read)
  def selectAll: Stream[ConnectionIO, DocumentRow] = sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from production.document""".query(using DocumentRow.read).stream
  def selectById(documentnode: DocumentId): ConnectionIO[Option[DocumentRow]] = sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from production.document where "documentnode" = ${fromWrite(documentnode)(Write.fromPut(DocumentId.put))}""".query(using DocumentRow.read).option
  def selectByIds(documentnodes: Array[DocumentId]): Stream[ConnectionIO, DocumentRow] = sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from production.document where "documentnode" = ANY(${fromWrite(documentnodes)(Write.fromPut(DocumentId.arrayPut))})""".query(using DocumentRow.read).stream
  ...
  def update: UpdateBuilder[DocumentFields, DocumentRow] = UpdateBuilder("production.document", DocumentFields.structure, DocumentRow.read)
