...
class UsersRepoImpl extends UsersRepo {
  ...
  def deleteById(userId: UsersId): ConnectionIO[Boolean] = sql"""delete from public.users where "user_id" = ${fromWrite(userId)(Write.fromPut(UsersId.put))}""".update.run.map(_ > 0)
  def deleteByIds(userIds: Array[UsersId]): ConnectionIO[Int] = sql"""delete from public.users where "user_id" = ANY(${fromWrite(userIds)(Write.fromPut(UsersId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, UsersRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY public.users("user_id", "name", "last_name", "email", "password", "created_at", "verified_on") FROM STDIN""").copyIn(unsaved, batchSize)(using UsersRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, UsersRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY public.users("user_id", "name", "last_name", "email", "password", "verified_on", "created_at") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using UsersRowUnsaved.text)
  def select: SelectBuilder[UsersFields, UsersRow] = SelectBuilderSql("public.users", UsersFields.structure, UsersRow.read)
  def selectAll: Stream[ConnectionIO, UsersRow] = sql"""select "user_id", "name", "last_name", "email"::text, "password", "created_at"::text, "verified_on"::text from public.users""".query(using UsersRow.read).stream
  def selectById(userId: UsersId): ConnectionIO[Option[UsersRow]] = sql"""select "user_id", "name", "last_name", "email"::text, "password", "created_at"::text, "verified_on"::text from public.users where "user_id" = ${fromWrite(userId)(Write.fromPut(UsersId.put))}""".query(using UsersRow.read).option
  def selectByIds(userIds: Array[UsersId]): Stream[ConnectionIO, UsersRow] = sql"""select "user_id", "name", "last_name", "email"::text, "password", "created_at"::text, "verified_on"::text from public.users where "user_id" = ANY(${fromWrite(userIds)(Write.fromPut(UsersId.arrayPut))})""".query(using UsersRow.read).stream
  ...
  def update: UpdateBuilder[UsersFields, UsersRow] = UpdateBuilder("public.users", UsersFields.structure, UsersRow.read)
