...
object TypoInstant {
  ...
  implicit lazy val arrayGet: Get[Array[TypoInstant]] = Get.Advanced.array[AnyRef](NonEmptyList.one("_timestamptz"))
    .map(_.map(v => TypoInstant(v.asInstanceOf[String])))
  implicit lazy val arrayPut: Put[Array[TypoInstant]] = Put.Advanced.array[AnyRef](NonEmptyList.one("_timestamptz"), "timestamptz")
