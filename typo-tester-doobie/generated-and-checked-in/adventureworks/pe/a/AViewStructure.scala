...
class AViewStructure[Row](val prefix: Option[String], val extract: Row => AViewRow, val merge: (Row, AViewRow) => Row)
  ...
  override val addressid = new Field[AddressId, Row](prefix, "addressid", None, Some("int4"))(x => extract(x).addressid, (row, value) => merge(row, extract(row).copy(addressid = value)))
  ...
  override val spatiallocation = new Field[Byte, Row](prefix, "spatiallocation", None, Some("bytea"))(x => extract(x).spatiallocation, (row, value) => merge(row, extract(row).copy(spatiallocation = value)))
