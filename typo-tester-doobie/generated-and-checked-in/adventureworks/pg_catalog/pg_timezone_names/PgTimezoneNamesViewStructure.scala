...
class PgTimezoneNamesViewStructure[Row](val prefix: Option[String], val extract: Row => PgTimezoneNamesViewRow, val merge: (Row, PgTimezoneNamesViewRow) => Row)
  ...
  override val abbrev = new Field[String, Row](prefix, "abbrev", None, None)(x => extract(x).abbrev, (row, value) => merge(row, extract(row).copy(abbrev = value)))
  override val utcOffset = new Field[TypoInterval, Row](prefix, "utc_offset", None, Some("interval"))(x => extract(x).utcOffset, (row, value) => merge(row, extract(row).copy(utcOffset = value)))
  override val isDst = new Field[Boolean, Row](prefix, "is_dst", None, None)(x => extract(x).isDst, (row, value) => merge(row, extract(row).copy(isDst = value)))
