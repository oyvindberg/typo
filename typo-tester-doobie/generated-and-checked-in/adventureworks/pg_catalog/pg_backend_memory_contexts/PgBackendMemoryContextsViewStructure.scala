...
class PgBackendMemoryContextsViewStructure[Row](val prefix: Option[String], val extract: Row => PgBackendMemoryContextsViewRow, val merge: (Row, PgBackendMemoryContextsViewRow) => Row)
  ...
  override val level = new OptField[Int, Row](prefix, "level", None, Some("int4"))(x => extract(x).level, (row, value) => merge(row, extract(row).copy(level = value)))
  ...
  override val totalNblocks = new OptField[Long, Row](prefix, "total_nblocks", None, Some("int8"))(x => extract(x).totalNblocks, (row, value) => merge(row, extract(row).copy(totalNblocks = value)))
  override val freeBytes = new OptField[Long, Row](prefix, "free_bytes", None, Some("int8"))(x => extract(x).freeBytes, (row, value) => merge(row, extract(row).copy(freeBytes = value)))
  override val freeChunks = new OptField[Long, Row](prefix, "free_chunks", None, Some("int8"))(x => extract(x).freeChunks, (row, value) => merge(row, extract(row).copy(freeChunks = value)))
  override val usedBytes = new OptField[Long, Row](prefix, "used_bytes", None, Some("int8"))(x => extract(x).usedBytes, (row, value) => merge(row, extract(row).copy(usedBytes = value)))
