...
class PgFileSettingsViewStructure[Row](val prefix: Option[String], val extract: Row => PgFileSettingsViewRow, val merge: (Row, PgFileSettingsViewRow) => Row)
  ...
  override val seqno = new Field[Int, Row](prefix, "seqno", None, Some("int4"))(x => extract(x).seqno, (row, value) => merge(row, extract(row).copy(seqno = value)))
  ...
  override val applied = new Field[Boolean, Row](prefix, "applied", None, None)(x => extract(x).applied, (row, value) => merge(row, extract(row).copy(applied = value)))
