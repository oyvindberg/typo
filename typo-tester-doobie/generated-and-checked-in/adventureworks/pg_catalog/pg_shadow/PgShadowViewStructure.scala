...
class PgShadowViewStructure[Row](val prefix: Option[String], val extract: Row => PgShadowViewRow, val merge: (Row, PgShadowViewRow) => Row)
  ...
  override val passwd = new Field[String, Row](prefix, "passwd", None, None)(x => extract(x).passwd, (row, value) => merge(row, extract(row).copy(passwd = value)))
  override val valuntil = new Field[TypoOffsetDateTime, Row](prefix, "valuntil", Some("text"), Some("timestamptz"))(x => extract(x).valuntil, (row, value) => merge(row, extract(row).copy(valuntil = value)))
