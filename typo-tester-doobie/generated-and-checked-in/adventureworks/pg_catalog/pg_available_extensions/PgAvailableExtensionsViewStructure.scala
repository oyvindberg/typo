...
class PgAvailableExtensionsViewStructure[Row](val prefix: Option[String], val extract: Row => PgAvailableExtensionsViewRow, val merge: (Row, PgAvailableExtensionsViewRow) => Row)
  ...
  override val defaultVersion = new Field[String, Row](prefix, "default_version", None, None)(x => extract(x).defaultVersion, (row, value) => merge(row, extract(row).copy(defaultVersion = value)))
