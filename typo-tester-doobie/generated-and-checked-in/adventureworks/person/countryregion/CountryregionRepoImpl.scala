...
class CountryregionRepoImpl extends CountryregionRepo {
  def delete: DeleteBuilder[CountryregionFields, CountryregionRow] = DeleteBuilder("person.countryregion", CountryregionFields.structure)
  def deleteById(countryregioncode: CountryregionId): ConnectionIO[Boolean] = sql"""delete from person.countryregion where "countryregioncode" = ${fromWrite(countryregioncode)(Write.fromPut(CountryregionId.put))}""".update.run.map(_ > 0)
  def deleteByIds(countryregioncodes: Array[CountryregionId]): ConnectionIO[Int] = sql"""delete from person.countryregion where "countryregioncode" = ANY(${fromWrite(countryregioncodes)(Write.fromPut(CountryregionId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, CountryregionRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.countryregion("countryregioncode", "name", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using CountryregionRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, CountryregionRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.countryregion("countryregioncode", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using CountryregionRowUnsaved.text)
  def select: SelectBuilder[CountryregionFields, CountryregionRow] = SelectBuilderSql("person.countryregion", CountryregionFields.structure, CountryregionRow.read)
  def selectAll: Stream[ConnectionIO, CountryregionRow] = sql"""select "countryregioncode", "name", "modifieddate"::text from person.countryregion""".query(using CountryregionRow.read).stream
  def selectById(countryregioncode: CountryregionId): ConnectionIO[Option[CountryregionRow]] = sql"""select "countryregioncode", "name", "modifieddate"::text from person.countryregion where "countryregioncode" = ${fromWrite(countryregioncode)(Write.fromPut(CountryregionId.put))}""".query(using CountryregionRow.read).option
  def selectByIds(countryregioncodes: Array[CountryregionId]): Stream[ConnectionIO, CountryregionRow] = sql"""select "countryregioncode", "name", "modifieddate"::text from person.countryregion where "countryregioncode" = ANY(${fromWrite(countryregioncodes)(Write.fromPut(CountryregionId.arrayPut))})""".query(using CountryregionRow.read).stream
  ...
  def update: UpdateBuilder[CountryregionFields, CountryregionRow] = UpdateBuilder("person.countryregion", CountryregionFields.structure, CountryregionRow.read)
