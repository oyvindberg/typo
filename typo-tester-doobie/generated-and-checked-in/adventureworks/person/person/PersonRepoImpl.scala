...
class PersonRepoImpl extends PersonRepo {
  def delete: DeleteBuilder[PersonFields, PersonRow] = DeleteBuilder("person.person", PersonFields.structure)
  def deleteById(businessentityid: BusinessentityId): ConnectionIO[Boolean] = sql"""delete from person.person where "businessentityid" = ${fromWrite(businessentityid)(Write.fromPut(BusinessentityId.put))}""".update.run.map(_ > 0)
  def deleteByIds(businessentityids: Array[BusinessentityId]): ConnectionIO[Int] = sql"""delete from person.person where "businessentityid" = ANY(${fromWrite(businessentityids)(Write.fromPut(BusinessentityId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, PersonRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.person("businessentityid", "persontype", "namestyle", "title", "firstname", "middlename", "lastname", "suffix", "emailpromotion", "additionalcontactinfo", "demographics", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using PersonRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, PersonRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.person("businessentityid", "persontype", "title", "firstname", "middlename", "lastname", "suffix", "additionalcontactinfo", "demographics", "namestyle", "emailpromotion", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using PersonRowUnsaved.text)
  def select: SelectBuilder[PersonFields, PersonRow] = SelectBuilderSql("person.person", PersonFields.structure, PersonRow.read)
  ...
  def update: UpdateBuilder[PersonFields, PersonRow] = UpdateBuilder("person.person", PersonFields.structure, PersonRow.read)
