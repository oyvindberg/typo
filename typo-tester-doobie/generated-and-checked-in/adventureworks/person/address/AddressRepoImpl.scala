...
class AddressRepoImpl extends AddressRepo {
  ...
  def deleteById(addressid: AddressId): ConnectionIO[Boolean] = sql"""delete from person.address where "addressid" = ${fromWrite(addressid)(Write.fromPut(AddressId.put))}""".update.run.map(_ > 0)
  def deleteByIds(addressids: Array[AddressId]): ConnectionIO[Int] = sql"""delete from person.address where "addressid" = ANY(${fromWrite(addressids)(Write.fromPut(AddressId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, AddressRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.address("addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using AddressRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, AddressRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.address("addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "addressid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using AddressRowUnsaved.text)
  def select: SelectBuilder[AddressFields, AddressRow] = SelectBuilderSql("person.address", AddressFields.structure, AddressRow.read)
  def selectAll: Stream[ConnectionIO, AddressRow] = sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from person.address""".query(using AddressRow.read).stream
  def selectById(addressid: AddressId): ConnectionIO[Option[AddressRow]] = sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from person.address where "addressid" = ${fromWrite(addressid)(Write.fromPut(AddressId.put))}""".query(using AddressRow.read).option
  def selectByIds(addressids: Array[AddressId]): Stream[ConnectionIO, AddressRow] = sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from person.address where "addressid" = ANY(${fromWrite(addressids)(Write.fromPut(AddressId.arrayPut))})""".query(using AddressRow.read).stream
  ...
  def update: UpdateBuilder[AddressFields, AddressRow] = UpdateBuilder("person.address", AddressFields.structure, AddressRow.read)
