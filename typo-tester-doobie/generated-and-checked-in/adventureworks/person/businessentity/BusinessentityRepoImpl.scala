...
class BusinessentityRepoImpl extends BusinessentityRepo {
  ...
  def deleteById(businessentityid: BusinessentityId): ConnectionIO[Boolean] = sql"""delete from person.businessentity where "businessentityid" = ${fromWrite(businessentityid)(Write.fromPut(BusinessentityId.put))}""".update.run.map(_ > 0)
  def deleteByIds(businessentityids: Array[BusinessentityId]): ConnectionIO[Int] = sql"""delete from person.businessentity where "businessentityid" = ANY(${fromWrite(businessentityids)(Write.fromPut(BusinessentityId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, BusinessentityRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.businessentity("businessentityid", "rowguid", "modifieddate") FROM STDIN""").copyIn(unsaved, batchSize)(using BusinessentityRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Stream[ConnectionIO, BusinessentityRowUnsaved], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY person.businessentity("businessentityid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""").copyIn(unsaved, batchSize)(using BusinessentityRowUnsaved.text)
  def select: SelectBuilder[BusinessentityFields, BusinessentityRow] = SelectBuilderSql("person.businessentity", BusinessentityFields.structure, BusinessentityRow.read)
  def selectAll: Stream[ConnectionIO, BusinessentityRow] = sql"""select "businessentityid", "rowguid", "modifieddate"::text from person.businessentity""".query(using BusinessentityRow.read).stream
  def selectById(businessentityid: BusinessentityId): ConnectionIO[Option[BusinessentityRow]] = sql"""select "businessentityid", "rowguid", "modifieddate"::text from person.businessentity where "businessentityid" = ${fromWrite(businessentityid)(Write.fromPut(BusinessentityId.put))}""".query(using BusinessentityRow.read).option
  def selectByIds(businessentityids: Array[BusinessentityId]): Stream[ConnectionIO, BusinessentityRow] = sql"""select "businessentityid", "rowguid", "modifieddate"::text from person.businessentity where "businessentityid" = ANY(${fromWrite(businessentityids)(Write.fromPut(BusinessentityId.arrayPut))})""".query(using BusinessentityRow.read).stream
  ...
  def update: UpdateBuilder[BusinessentityFields, BusinessentityRow] = UpdateBuilder("person.businessentity", BusinessentityFields.structure, BusinessentityRow.read)
