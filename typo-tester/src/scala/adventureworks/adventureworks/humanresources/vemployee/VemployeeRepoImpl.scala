...
object VemployeeRepoImpl extends VemployeeRepo {
  override def selectAll(implicit c: Connection): List[VemployeeRow] = {
    SQL"""select businessentityid, title, firstname, middlename, lastname, suffix, jobtitle, phonenumber, phonenumbertype, emailaddress, emailpromotion, addressline1, addressline2, city, stateprovincename, postalcode, countryregionname, additionalcontactinfo from humanresources.vemployee""".as(VemployeeRow.rowParser("").*)
  }
