...
object ProductdescriptionRepoImpl extends ProductdescriptionRepo {
  ...
  override def insert(unsaved: ProductdescriptionRowUnsaved)(implicit c: Connection): ProductdescriptionRow = {
    ...
    if (namedParameters.isEmpty) {
      ...
      val q = s"""insert into production.productdescription(${namedParameters.map{case (x, _) => "\"" + x.name + "\""}.mkString(", ")})
  ...
  override def selectByFieldValues(fieldValues: List[ProductdescriptionFieldOrIdValue[_]])(implicit c: Connection): List[ProductdescriptionRow] = {
    fieldValues match {
      ...
      case nonEmpty =>
        ...
        val q = s"""select productdescriptionid, description, rowguid, modifieddate
                    from production.productdescription
  ...
  override def updateFieldValues(productdescriptionid: ProductdescriptionId, fieldValues: List[ProductdescriptionFieldValue[_]])(implicit c: Connection): Boolean = {
    fieldValues match {
      ...
      case nonEmpty =>
        ...
        SQL(q)
          .on(namedParams: _*)
          .on(NamedParameter("productdescriptionid", ParameterValue.from(productdescriptionid)))
          .executeUpdate() > 0
    }
