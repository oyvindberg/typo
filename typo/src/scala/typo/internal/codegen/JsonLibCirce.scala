  ...
  def lookupDecoderFor(tpe: sc.Type): sc.Code = {
    def go(tpe: sc.Type): sc.Code =
      tpe match {
        ...
        case sc.Type.TApply(TypesScala.Array, List(targ))                  => code"$Decoder.decodeArray[$targ](${go(targ)}, implicitly)"
  ...
  def lookupEncoderFor(tpe: sc.Type): sc.Code = {
    def go(tpe: sc.Type): sc.Code =
      tpe match {
        ...
        case sc.Type.TApply(TypesScala.Array, List(targ))                  => code"$Encoder.encodeIterable[$targ, ${TypesScala.Array}](${go(targ)}, implicitly)"
