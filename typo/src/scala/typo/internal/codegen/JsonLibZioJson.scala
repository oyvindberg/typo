  ...
  private def lookupDecoderFor(tpe: sc.Type): sc.Code = {
    def go(tpe: sc.Type): sc.Code =
      tpe match {
        ...
        case sc.Type.TApply(TypesScala.Array, List(targ))                  => code"$JsonDecoder.array[$targ](${go(targ)}, implicitly)"
  ...
  def lookupEncoderFor(tpe: sc.Type): sc.Code = {
    def go(tpe: sc.Type): sc.Code =
      tpe match {
        ...
        case sc.Type.TApply(TypesScala.Array, List(targ))                  => code"$JsonEncoder.array[$targ](${go(targ)}, implicitly)"
