...
class RoleTableGrantsViewStructure[Row](val prefix: Option[String], val extract: Row => RoleTableGrantsViewRow, val merge: (Row, RoleTableGrantsViewRow) => Row)
  extends Relation[RoleTableGrantsViewFields, RoleTableGrantsViewRow, Row]
    with RoleTableGrantsViewFields[Row] { outer =>
  ...
  override val withHierarchy = new OptField[/* nullability unknown */ /* max 3 chars */ String, Row](prefix, "with_hierarchy", None, None)(x => extract(x).withHierarchy, (row, value) => merge(row, extract(row).copy(withHierarchy = value)))
  ...
  override def copy[NewRow](prefix: Option[String], extract: NewRow => RoleTableGrantsViewRow, merge: (NewRow, RoleTableGrantsViewRow) => NewRow): RoleTableGrantsViewStructure[NewRow] =
    new RoleTableGrantsViewStructure(prefix, extract, merge)
}
