...
class CurrencyRepoImpl extends CurrencyRepo {
  ...
  def deleteById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from sales.currency where "currencycode" = ${Segment.paramSegment(currencycode)(CurrencyId.setter)}""".delete.map(_ > 0)
  def deleteByIds(currencycodes: Array[CurrencyId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from sales.currency where "currencycode" = ANY(${Segment.paramSegment(currencycodes)(CurrencyId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, CurrencyRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY sales.currency("currencycode", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(CurrencyRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, CurrencyRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY sales.currency("currencycode", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(CurrencyRowUnsaved.text)
  def select: SelectBuilder[CurrencyFields, CurrencyRow] = SelectBuilderSql("sales.currency", CurrencyFields.structure, CurrencyRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, CurrencyRow] = sql"""select "currencycode", "name", "modifieddate"::text from sales.currency""".query(using CurrencyRow.jdbcDecoder).selectStream()
  def selectById(currencycode: CurrencyId): ZIO[ZConnection, Throwable, Option[CurrencyRow]] = sql"""select "currencycode", "name", "modifieddate"::text from sales.currency where "currencycode" = ${Segment.paramSegment(currencycode)(CurrencyId.setter)}""".query(using CurrencyRow.jdbcDecoder).selectOne
  def selectByIds(currencycodes: Array[CurrencyId]): ZStream[ZConnection, Throwable, CurrencyRow] = sql"""select "currencycode", "name", "modifieddate"::text from sales.currency where "currencycode" = ANY(${Segment.paramSegment(currencycodes)(CurrencyId.arraySetter)})""".query(using CurrencyRow.jdbcDecoder).selectStream()
  ...
  def update: UpdateBuilder[CurrencyFields, CurrencyRow] = UpdateBuilder("sales.currency", CurrencyFields.structure, CurrencyRow.jdbcDecoder)
