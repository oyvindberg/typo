...
class CustomerRepoImpl extends CustomerRepo {
  ...
  def deleteById(customerid: CustomerId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from sales.customer where "customerid" = ${Segment.paramSegment(customerid)(CustomerId.setter)}""".delete.map(_ > 0)
  def deleteByIds(customerids: Array[CustomerId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from sales.customer where "customerid" = ANY(${Segment.paramSegment(customerids)(CustomerId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, CustomerRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY sales.customer("customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(CustomerRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, CustomerRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY sales.customer("personid", "storeid", "territoryid", "customerid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(CustomerRowUnsaved.text)
  def select: SelectBuilder[CustomerFields, CustomerRow] = SelectBuilderSql("sales.customer", CustomerFields.structure, CustomerRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, CustomerRow] = sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from sales.customer""".query(using CustomerRow.jdbcDecoder).selectStream()
  def selectById(customerid: CustomerId): ZIO[ZConnection, Throwable, Option[CustomerRow]] = sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from sales.customer where "customerid" = ${Segment.paramSegment(customerid)(CustomerId.setter)}""".query(using CustomerRow.jdbcDecoder).selectOne
  def selectByIds(customerids: Array[CustomerId]): ZStream[ZConnection, Throwable, CustomerRow] = sql"""select "customerid", "personid", "storeid", "territoryid", "rowguid", "modifieddate"::text from sales.customer where "customerid" = ANY(${Segment.paramSegment(customerids)(CustomerId.arraySetter)})""".query(using CustomerRow.jdbcDecoder).selectStream()
  ...
  def update: UpdateBuilder[CustomerFields, CustomerRow] = UpdateBuilder("sales.customer", CustomerFields.structure, CustomerRow.jdbcDecoder)
