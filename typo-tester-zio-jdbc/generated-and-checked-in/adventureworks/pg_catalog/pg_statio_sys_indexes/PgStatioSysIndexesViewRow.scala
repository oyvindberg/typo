...
object PgStatioSysIndexesViewRow {
  implicit lazy val jdbcDecoder: JdbcDecoder[PgStatioSysIndexesViewRow] = new JdbcDecoder[PgStatioSysIndexesViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PgStatioSysIndexesViewRow) =
      columIndex + 6 ->
  ...
  implicit lazy val jsonDecoder: JsonDecoder[PgStatioSysIndexesViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    if (relid.isRight && indexrelid.isRight && schemaname.isRight && relname.isRight && indexrelname.isRight && idxBlksRead.isRight && idxBlksHit.isRight)
      Right(PgStatioSysIndexesViewRow(relid = relid.toOption.get, indexrelid = indexrelid.toOption.get, schemaname = schemaname.toOption.get, relname = relname.toOption.get, indexrelname = indexrelname.toOption.get, idxBlksRead = idxBlksRead.toOption.get, idxBlksHit = idxBlksHit.toOption.get))
  ...
  implicit lazy val jsonEncoder: JsonEncoder[PgStatioSysIndexesViewRow] = new JsonEncoder[PgStatioSysIndexesViewRow] {
    override def unsafeEncode(a: PgStatioSysIndexesViewRow, indent: Option[Int], out: Write): Unit = {
      out.write("{")
      out.write(""""relid":""")
