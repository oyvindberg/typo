...
class PgTypeRepoMock(map: scala.collection.mutable.Map[PgTypeId, PgTypeRow] = scala.collection.mutable.Map.empty) extends PgTypeRepo {
  ...
  override def select: SelectBuilder[PgTypeFields, PgTypeRow] = {
    SelectBuilderMock(PgTypeFields, ZIO.succeed(Chunk.fromIterable(map.values)), SelectParams.empty)
  }
  ...
  override def selectByUnique(typname: String, typnamespace: /* oid */ Long): ZIO[ZConnection, Throwable, Option[PgTypeRow]] = {
    ZIO.succeed(map.values.find(v => typname == v.typname && typnamespace == v.typnamespace))
  }
