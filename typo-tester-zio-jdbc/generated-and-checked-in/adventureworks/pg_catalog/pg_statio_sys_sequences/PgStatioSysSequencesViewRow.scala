...
object PgStatioSysSequencesViewRow {
  implicit lazy val jdbcDecoder: JdbcDecoder[PgStatioSysSequencesViewRow] = new JdbcDecoder[PgStatioSysSequencesViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PgStatioSysSequencesViewRow) =
      columIndex + 4 ->
  ...
  implicit lazy val jsonDecoder: JsonDecoder[PgStatioSysSequencesViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    if (relid.isRight && schemaname.isRight && relname.isRight && blksRead.isRight && blksHit.isRight)
      Right(PgStatioSysSequencesViewRow(relid = relid.toOption.get, schemaname = schemaname.toOption.get, relname = relname.toOption.get, blksRead = blksRead.toOption.get, blksHit = blksHit.toOption.get))
    else Left(List[Either[String, Any]](relid, schemaname, relname, blksRead, blksHit).flatMap(_.left.toOption).mkString(", "))
  }
  implicit lazy val jsonEncoder: JsonEncoder[PgStatioSysSequencesViewRow] = new JsonEncoder[PgStatioSysSequencesViewRow] {
    override def unsafeEncode(a: PgStatioSysSequencesViewRow, indent: Option[Int], out: Write): Unit = {
      out.write("{")
      out.write(""""relid":""")
