...
class PgConstraintRepoMock(map: scala.collection.mutable.Map[PgConstraintId, PgConstraintRow] = scala.collection.mutable.Map.empty) extends PgConstraintRepo {
  ...
  override def select: SelectBuilder[PgConstraintFields, PgConstraintRow] = {
    SelectBuilderMock(PgConstraintFields, ZIO.succeed(Chunk.fromIterable(map.values)), SelectParams.empty)
  }
  ...
  override def selectByUnique(conrelid: /* oid */ Long, contypid: /* oid */ Long, conname: String): ZIO[ZConnection, Throwable, Option[PgConstraintRow]] = {
    ZIO.succeed(map.values.find(v => conrelid == v.conrelid && contypid == v.contypid && conname == v.conname))
  }
