...
object PgStatUserIndexesViewRow {
  implicit lazy val jdbcDecoder: JdbcDecoder[PgStatUserIndexesViewRow] = new JdbcDecoder[PgStatUserIndexesViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PgStatUserIndexesViewRow) =
      columIndex + 7 ->
  ...
  implicit lazy val jsonDecoder: JsonDecoder[PgStatUserIndexesViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    if (relid.isRight && indexrelid.isRight && schemaname.isRight && relname.isRight && indexrelname.isRight && idxScan.isRight && idxTupRead.isRight && idxTupFetch.isRight)
      Right(PgStatUserIndexesViewRow(relid = relid.toOption.get, indexrelid = indexrelid.toOption.get, schemaname = schemaname.toOption.get, relname = relname.toOption.get, indexrelname = indexrelname.toOption.get, idxScan = idxScan.toOption.get, idxTupRead = idxTupRead.toOption.get, idxTupFetch = idxTupFetch.toOption.get))
  ...
  implicit lazy val jsonEncoder: JsonEncoder[PgStatUserIndexesViewRow] = new JsonEncoder[PgStatUserIndexesViewRow] {
    override def unsafeEncode(a: PgStatUserIndexesViewRow, indent: Option[Int], out: Write): Unit = {
      out.write("{")
      out.write(""""relid":""")
