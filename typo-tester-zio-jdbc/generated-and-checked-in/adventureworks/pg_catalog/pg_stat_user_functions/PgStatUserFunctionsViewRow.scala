...
object PgStatUserFunctionsViewRow {
  implicit lazy val jdbcDecoder: JdbcDecoder[PgStatUserFunctionsViewRow] = new JdbcDecoder[PgStatUserFunctionsViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PgStatUserFunctionsViewRow) =
      columIndex + 5 ->
  ...
  implicit lazy val jsonDecoder: JsonDecoder[PgStatUserFunctionsViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    if (funcid.isRight && schemaname.isRight && funcname.isRight && calls.isRight && totalTime.isRight && selfTime.isRight)
      Right(PgStatUserFunctionsViewRow(funcid = funcid.toOption.get, schemaname = schemaname.toOption.get, funcname = funcname.toOption.get, calls = calls.toOption.get, totalTime = totalTime.toOption.get, selfTime = selfTime.toOption.get))
    else Left(List[Either[String, Any]](funcid, schemaname, funcname, calls, totalTime, selfTime).flatMap(_.left.toOption).mkString(", "))
  }
  implicit lazy val jsonEncoder: JsonEncoder[PgStatUserFunctionsViewRow] = new JsonEncoder[PgStatUserFunctionsViewRow] {
    override def unsafeEncode(a: PgStatUserFunctionsViewRow, indent: Option[Int], out: Write): Unit = {
      out.write("{")
      out.write(""""funcid":""")
