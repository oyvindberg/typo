...
class PgSettingsViewStructure[Row](val prefix: Option[String], val extract: Row => PgSettingsViewRow, val merge: (Row, PgSettingsViewRow) => Row)
  ...
  override val enumvals = new OptField[Array[String], Row](prefix, "enumvals", None, None)(x => extract(x).enumvals, (row, value) => merge(row, extract(row).copy(enumvals = value)))
  ...
  override val sourceline = new OptField[Int, Row](prefix, "sourceline", None, None)(x => extract(x).sourceline, (row, value) => merge(row, extract(row).copy(sourceline = value)))
