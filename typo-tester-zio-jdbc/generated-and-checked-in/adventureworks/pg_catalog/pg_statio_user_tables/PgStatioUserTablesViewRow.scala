...
object PgStatioUserTablesViewRow {
  implicit lazy val jdbcDecoder: JdbcDecoder[PgStatioUserTablesViewRow] = new JdbcDecoder[PgStatioUserTablesViewRow] {
    override def unsafeDecode(columIndex: Int, rs: ResultSet): (Int, PgStatioUserTablesViewRow) =
      columIndex + 10 ->
  ...
  implicit lazy val jsonDecoder: JsonDecoder[PgStatioUserTablesViewRow] = JsonDecoder[Json.Obj].mapOrFail { jsonObj =>
    ...
    if (relid.isRight && schemaname.isRight && relname.isRight && heapBlksRead.isRight && heapBlksHit.isRight && idxBlksRead.isRight && idxBlksHit.isRight && toastBlksRead.isRight && toastBlksHit.isRight && tidxBlksRead.isRight && tidxBlksHit.isRight)
      Right(PgStatioUserTablesViewRow(relid = relid.toOption.get, schemaname = schemaname.toOption.get, relname = relname.toOption.get, heapBlksRead = heapBlksRead.toOption.get, heapBlksHit = heapBlksHit.toOption.get, idxBlksRead = idxBlksRead.toOption.get, idxBlksHit = idxBlksHit.toOption.get, toastBlksRead = toastBlksRead.toOption.get, toastBlksHit = toastBlksHit.toOption.get, tidxBlksRead = tidxBlksRead.toOption.get, tidxBlksHit = tidxBlksHit.toOption.get))
  ...
  implicit lazy val jsonEncoder: JsonEncoder[PgStatioUserTablesViewRow] = new JsonEncoder[PgStatioUserTablesViewRow] {
    override def unsafeEncode(a: PgStatioUserTablesViewRow, indent: Option[Int], out: Write): Unit = {
      out.write("{")
      out.write(""""relid":""")
