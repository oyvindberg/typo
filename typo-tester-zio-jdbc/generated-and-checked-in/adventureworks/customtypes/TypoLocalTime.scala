...
object TypoLocalTime {
  ...
  implicit lazy val arrayJdbcDecoder: JdbcDecoder[Array[TypoLocalTime]] = {
    JdbcDecoder[Array[TypoLocalTime]]((rs: ResultSet) => (i: Int) =>
      rs.getArray(i) match {
        case null => null
        case arr => arr.getArray.asInstanceOf[Array[AnyRef]].map(x => new TypoLocalTime(LocalTime.parse(x.asInstanceOf[String])))
      },
      "Array[java.lang.String]"
    )
  }
  implicit lazy val arrayJdbcEncoder: JdbcEncoder[Array[TypoLocalTime]] = JdbcEncoder.singleParamEncoder(using arraySetter)
  implicit lazy val arraySetter: Setter[Array[TypoLocalTime]] = {
  ...
  implicit lazy val jdbcDecoder: JdbcDecoder[TypoLocalTime] = {
    JdbcDecoder[TypoLocalTime](
      (rs: ResultSet) => (i: Int) => {
        ...
        if (v eq null) null else new TypoLocalTime(LocalTime.parse(v.asInstanceOf[String]))
      },
      "java.lang.String"
    )
  }
  ...
  implicit lazy val setter: Setter[TypoLocalTime] = {
    Setter.other(
      (ps, i, v) => {
        ps.setObject(
          i,
