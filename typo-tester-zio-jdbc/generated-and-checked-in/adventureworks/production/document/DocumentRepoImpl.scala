...
class DocumentRepoImpl extends DocumentRepo {
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, DocumentRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY production.document("title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate", "documentnode") FROM STDIN""", batchSize, unsaved)(DocumentRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, DocumentRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY production.document("title", "owner", "filename", "fileextension", "revision", "status", "documentsummary", "document", "folderflag", "changenumber", "rowguid", "modifieddate", "documentnode") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(DocumentRowUnsaved.text)
  def select: SelectBuilder[DocumentFields, DocumentRow] = SelectBuilderSql("production.document", DocumentFields.structure, DocumentRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, DocumentRow] = sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from production.document""".query(using DocumentRow.jdbcDecoder).selectStream()
  def selectById(documentnode: DocumentId): ZIO[ZConnection, Throwable, Option[DocumentRow]] = sql"""select "title", "owner", "folderflag", "filename", "fileextension", "revision", "changenumber", "status", "documentsummary", "document", "rowguid", "modifieddate"::text, "documentnode" from production.document where "documentnode" = ${Segment.paramSegment(documentnode)(DocumentId.setter)}""".query(using DocumentRow.jdbcDecoder).selectOne
  ...
  def update: UpdateBuilder[DocumentFields, DocumentRow] = UpdateBuilder("production.document", DocumentFields.structure, DocumentRow.jdbcDecoder)
