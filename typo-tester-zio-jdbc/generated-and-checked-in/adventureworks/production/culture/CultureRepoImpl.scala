...
class CultureRepoImpl extends CultureRepo {
  ...
  def deleteById(cultureid: CultureId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from production.culture where "cultureid" = ${Segment.paramSegment(cultureid)(CultureId.setter)}""".delete.map(_ > 0)
  def deleteByIds(cultureids: Array[CultureId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from production.culture where "cultureid" = ANY(${Segment.paramSegment(cultureids)(CultureId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, CultureRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY production.culture("cultureid", "name", "modifieddate") FROM STDIN""", batchSize, unsaved)(CultureRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, CultureRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY production.culture("cultureid", "name", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(CultureRowUnsaved.text)
  def select: SelectBuilder[CultureFields, CultureRow] = SelectBuilderSql("production.culture", CultureFields.structure, CultureRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, CultureRow] = sql"""select "cultureid", "name", "modifieddate"::text from production.culture""".query(using CultureRow.jdbcDecoder).selectStream()
  def selectById(cultureid: CultureId): ZIO[ZConnection, Throwable, Option[CultureRow]] = sql"""select "cultureid", "name", "modifieddate"::text from production.culture where "cultureid" = ${Segment.paramSegment(cultureid)(CultureId.setter)}""".query(using CultureRow.jdbcDecoder).selectOne
  def selectByIds(cultureids: Array[CultureId]): ZStream[ZConnection, Throwable, CultureRow] = sql"""select "cultureid", "name", "modifieddate"::text from production.culture where "cultureid" = ANY(${Segment.paramSegment(cultureids)(CultureId.arraySetter)})""".query(using CultureRow.jdbcDecoder).selectStream()
  ...
  def update: UpdateBuilder[CultureFields, CultureRow] = UpdateBuilder("production.culture", CultureFields.structure, CultureRow.jdbcDecoder)
