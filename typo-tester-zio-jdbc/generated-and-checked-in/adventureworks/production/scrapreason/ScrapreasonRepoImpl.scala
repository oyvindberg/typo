...
object ScrapreasonRepoImpl extends ScrapreasonRepo {
  override def delete(scrapreasonid: ScrapreasonId): ZIO[ZConnection, Throwable, Boolean] = {
    sql"""delete from production.scrapreason where "scrapreasonid" = ${Segment.paramSegment(scrapreasonid)(Setter[ScrapreasonId])}""".delete.map(_ > 0)
  }
  ...
  override def insert(unsaved: ScrapreasonRow): ZIO[ZConnection, Throwable, ScrapreasonRow] = {
    sql"""insert into production.scrapreason("scrapreasonid", "name", "modifieddate")
          values (${Segment.paramSegment(unsaved.scrapreasonid)(Setter[ScrapreasonId])}::int4, ${Segment.paramSegment(unsaved.name)(Setter[Name])}::varchar, ${Segment.paramSegment(unsaved.modifieddate)(Setter[TypoLocalDateTime])}::timestamp)
  ...
  override def insert(unsaved: ScrapreasonRowUnsaved): ZIO[ZConnection, Throwable, ScrapreasonRow] = {
    val fs = List(
      ...
      unsaved.scrapreasonid match {
        ...
        case Defaulted.Provided(value) => Some((sql""""scrapreasonid"""", sql"${Segment.paramSegment(value: ScrapreasonId)(Setter[ScrapreasonId])}::int4"))
      },
  ...
  override def selectById(scrapreasonid: ScrapreasonId): ZIO[ZConnection, Throwable, Option[ScrapreasonRow]] = {
    sql"""select "scrapreasonid", "name", "modifieddate"::text from production.scrapreason where "scrapreasonid" = ${Segment.paramSegment(scrapreasonid)(Setter[ScrapreasonId])}""".query(ScrapreasonRow.jdbcDecoder).selectOne
  }
  ...
  override def update(row: ScrapreasonRow): ZIO[ZConnection, Throwable, Boolean] = {
    ...
    sql"""update production.scrapreason
          ...
          where "scrapreasonid" = ${Segment.paramSegment(scrapreasonid)(Setter[ScrapreasonId])}""".update.map(_ > 0)
  }
  ...
  override def upsert(unsaved: ScrapreasonRow): ZIO[ZConnection, Throwable, UpdateResult[ScrapreasonRow]] = {
    sql"""insert into production.scrapreason("scrapreasonid", "name", "modifieddate")
          values (
            ${Segment.paramSegment(unsaved.scrapreasonid)(Setter[ScrapreasonId])}::int4,
