...
class LocationRepoImpl extends LocationRepo {
  ...
  def deleteById(locationid: LocationId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from production.location where "locationid" = ${Segment.paramSegment(locationid)(LocationId.setter)}""".delete.map(_ > 0)
  def deleteByIds(locationids: Array[LocationId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from production.location where "locationid" = ANY(${Segment.paramSegment(locationids)(LocationId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, LocationRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY production.location("locationid", "name", "costrate", "availability", "modifieddate") FROM STDIN""", batchSize, unsaved)(LocationRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, LocationRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY production.location("name", "locationid", "costrate", "availability", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(LocationRowUnsaved.text)
  def select: SelectBuilder[LocationFields, LocationRow] = SelectBuilderSql("production.location", LocationFields.structure, LocationRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, LocationRow] = sql"""select "locationid", "name", "costrate", "availability", "modifieddate"::text from production.location""".query(using LocationRow.jdbcDecoder).selectStream()
  def selectById(locationid: LocationId): ZIO[ZConnection, Throwable, Option[LocationRow]] = sql"""select "locationid", "name", "costrate", "availability", "modifieddate"::text from production.location where "locationid" = ${Segment.paramSegment(locationid)(LocationId.setter)}""".query(using LocationRow.jdbcDecoder).selectOne
  def selectByIds(locationids: Array[LocationId]): ZStream[ZConnection, Throwable, LocationRow] = sql"""select "locationid", "name", "costrate", "availability", "modifieddate"::text from production.location where "locationid" = ANY(${Segment.paramSegment(locationids)(LocationId.arraySetter)})""".query(using LocationRow.jdbcDecoder).selectStream()
  ...
  def update: UpdateBuilder[LocationFields, LocationRow] = UpdateBuilder("production.location", LocationFields.structure, LocationRow.jdbcDecoder)
