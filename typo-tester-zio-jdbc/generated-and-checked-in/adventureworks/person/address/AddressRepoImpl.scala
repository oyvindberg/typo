...
class AddressRepoImpl extends AddressRepo {
  def delete: DeleteBuilder[AddressFields, AddressRow] = DeleteBuilder("person.address", AddressFields.structure)
  def deleteById(addressid: AddressId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from person.address where "addressid" = ${Segment.paramSegment(addressid)(AddressId.setter)}""".delete.map(_ > 0)
  def deleteByIds(addressids: Array[AddressId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from person.address where "addressid" = ANY(${Segment.paramSegment(addressids)(AddressId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, AddressRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY person.address("addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate") FROM STDIN""", batchSize, unsaved)(AddressRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, AddressRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY person.address("addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "addressid", "rowguid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(AddressRowUnsaved.text)
  def select: SelectBuilder[AddressFields, AddressRow] = SelectBuilderSql("person.address", AddressFields.structure, AddressRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, AddressRow] = sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from person.address""".query(using AddressRow.jdbcDecoder).selectStream()
  def selectById(addressid: AddressId): ZIO[ZConnection, Throwable, Option[AddressRow]] = sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from person.address where "addressid" = ${Segment.paramSegment(addressid)(AddressId.setter)}""".query(using AddressRow.jdbcDecoder).selectOne
  def selectByIds(addressids: Array[AddressId]): ZStream[ZConnection, Throwable, AddressRow] = sql"""select "addressid", "addressline1", "addressline2", "city", "stateprovinceid", "postalcode", "spatiallocation", "rowguid", "modifieddate"::text from person.address where "addressid" = ANY(${Segment.paramSegment(addressids)(AddressId.arraySetter)})""".query(using AddressRow.jdbcDecoder).selectStream()
  ...
  def update: UpdateBuilder[AddressFields, AddressRow] = UpdateBuilder("person.address", AddressFields.structure, AddressRow.jdbcDecoder)
