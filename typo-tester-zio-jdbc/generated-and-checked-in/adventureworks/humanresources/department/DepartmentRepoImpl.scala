...
class DepartmentRepoImpl extends DepartmentRepo {
  ...
  def deleteById(departmentid: DepartmentId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from humanresources.department where "departmentid" = ${Segment.paramSegment(departmentid)(DepartmentId.setter)}""".delete.map(_ > 0)
  def deleteByIds(departmentids: Array[DepartmentId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from humanresources.department where "departmentid" = ANY(${Segment.paramSegment(departmentids)(DepartmentId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, DepartmentRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY humanresources.department("departmentid", "name", "groupname", "modifieddate") FROM STDIN""", batchSize, unsaved)(DepartmentRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, DepartmentRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY humanresources.department("name", "groupname", "departmentid", "modifieddate") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(DepartmentRowUnsaved.text)
  def select: SelectBuilder[DepartmentFields, DepartmentRow] = SelectBuilderSql("humanresources.department", DepartmentFields.structure, DepartmentRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, DepartmentRow] = sql"""select "departmentid", "name", "groupname", "modifieddate"::text from humanresources.department""".query(using DepartmentRow.jdbcDecoder).selectStream()
  def selectById(departmentid: DepartmentId): ZIO[ZConnection, Throwable, Option[DepartmentRow]] = sql"""select "departmentid", "name", "groupname", "modifieddate"::text from humanresources.department where "departmentid" = ${Segment.paramSegment(departmentid)(DepartmentId.setter)}""".query(using DepartmentRow.jdbcDecoder).selectOne
  ...
  def update: UpdateBuilder[DepartmentFields, DepartmentRow] = UpdateBuilder("humanresources.department", DepartmentFields.structure, DepartmentRow.jdbcDecoder)
