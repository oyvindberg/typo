...
case class PersonWithAddressesRepo(
  ...
  /** A person can have a bunch of addresses registered, and they each have their own type.
    ...
    */
  def upsert(pa: PersonWithAddresses): ZIO[ZConnection, Throwable, List[BusinessentityaddressRow]] = {
    for {
      // update person
      _ <- personRepo.upsert(pa.person)
      // update stored addresses
      _ <- pa.addresses.values.toList.forEach { address => addressRepo.update(address) }
      ...
      currentAddressesWithAddresstype <- pa.addresses.toList.forEach { case (addressTypeName, wanted) =>
