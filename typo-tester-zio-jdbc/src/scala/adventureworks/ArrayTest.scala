  ...
  test("can query pgtestnull with DSL") {
    withConnection {
      ...
      for {
        ...
        _ <- pgtestnullRepo.select.where(_.bool === row.bool).toChunk.map(x => assertJsonEquals(row.bool, x.head.bool))
        _ <- pgtestnullRepo.select.where(_.box === row.box).toChunk.map(x => assertJsonEquals(row.box, x.head.box))
        _ <- pgtestnullRepo.select.where(_.bpchar === row.bpchar).toChunk.map(x => assertJsonEquals(row.bpchar, x.head.bpchar))
        _ <- pgtestnullRepo.select.where(_.bytea === row.bytea).toChunk.map(x => assertJsonEquals(row.bytea, x.head.bytea))
        _ <- pgtestnullRepo.select.where(_.char === row.char).toChunk.map(x => assertJsonEquals(row.char, x.head.char))
        _ <- pgtestnullRepo.select.where(_.circle === row.circle).toChunk.map(x => assertJsonEquals(row.circle, x.head.circle))
        _ <- pgtestnullRepo.select.where(_.date === row.date).toChunk.map(x => assertJsonEquals(row.date, x.head.date))
        _ <- pgtestnullRepo.select.where(_.float4 === row.float4).toChunk.map(x => assertJsonEquals(row.float4, x.head.float4))
        _ <- pgtestnullRepo.select.where(_.float8 === row.float8).toChunk.map(x => assertJsonEquals(row.float8, x.head.float8))
        _ <- pgtestnullRepo.select.where(_.hstore === row.hstore).toChunk.map(x => assertJsonEquals(row.hstore, x.head.hstore))
        _ <- pgtestnullRepo.select.where(_.inet === row.inet).toChunk.map(x => assertJsonEquals(row.inet, x.head.inet))
        _ <- pgtestnullRepo.select.where(_.int2 === row.int2).toChunk.map(x => assertJsonEquals(row.int2, x.head.int2))
        _ <- pgtestnullRepo.select.where(_.int2vector === row.int2vector).toChunk.map(x => assertJsonEquals(row.int2vector, x.head.int2vector))
        _ <- pgtestnullRepo.select.where(_.int4 === row.int4).toChunk.map(x => assertJsonEquals(row.int4, x.head.int4))
        _ <- pgtestnullRepo.select.where(_.int8 === row.int8).toChunk.map(x => assertJsonEquals(row.int8, x.head.int8))
        _ <- pgtestnullRepo.select.where(_.interval === row.interval).toChunk.map(x => assertJsonEquals(row.interval, x.head.interval))
//        _ <- pgtestnullRepo.select.where(_.json === row.json).toChunk.map(x => assertJsonEquals(row.json, x.head.json))
        _ <- pgtestnullRepo.select.where(_.jsonb === row.jsonb).toChunk.map(x => assertJsonEquals(row.jsonb, x.head.jsonb))
        _ <- pgtestnullRepo.select.where(_.line === row.line).toChunk.map(x => assertJsonEquals(row.line, x.head.line))
        _ <- pgtestnullRepo.select.where(_.lseg === row.lseg).toChunk.map(x => assertJsonEquals(row.lseg, x.head.lseg))
        _ <- pgtestnullRepo.select.where(_.money === row.money).toChunk.map(x => assertJsonEquals(row.money, x.head.money))
        _ <- pgtestnullRepo.select.where(_.mydomain === row.mydomain).toChunk.map(x => assertJsonEquals(row.mydomain, x.head.mydomain))
        _ <- pgtestnullRepo.select.where(_.myenum === row.myenum).toChunk.map(x => assertJsonEquals(row.myenum, x.head.myenum))
        _ <- pgtestnullRepo.select.where(_.name === row.name).toChunk.map(x => assertJsonEquals(row.name, x.head.name))
        _ <- pgtestnullRepo.select.where(_.numeric === row.numeric).toChunk.map(x => assertJsonEquals(row.numeric, x.head.numeric))
        _ <- pgtestnullRepo.select.where(_.path === row.path).toChunk.map(x => assertJsonEquals(row.path, x.head.path))
//        _ <- pgtestnullRepo.select.where(_.point === row.point).toChunk.map(x => assertJsonEquals(row.point, x.head.point))
//        _ <- pgtestnullRepo.select.where(_.polygon === row.polygon).toChunk.map(x => assertJsonEquals(row.polygon, x.head.polygon))
        _ <- pgtestnullRepo.select.where(_.text === row.text).toChunk.map(x => assertJsonEquals(row.text, x.head.text))
        _ <- pgtestnullRepo.select.where(_.time === row.time).toChunk.map(x => assertJsonEquals(row.time, x.head.time))
        _ <- pgtestnullRepo.select.where(_.timestamp === row.timestamp).toChunk.map(x => assertJsonEquals(row.timestamp, x.head.timestamp))
        _ <- pgtestnullRepo.select.where(_.timestampz === row.timestampz).toChunk.map(x => assertJsonEquals(row.timestampz, x.head.timestampz))
        _ <- pgtestnullRepo.select.where(_.timez === row.timez).toChunk.map(x => assertJsonEquals(row.timez, x.head.timez))
        _ <- pgtestnullRepo.select.where(_.uuid === row.uuid).toChunk.map(x => assertJsonEquals(row.uuid, x.head.uuid))
        _ <- pgtestnullRepo.select.where(_.varchar === row.varchar).toChunk.map(x => assertJsonEquals(row.varchar, x.head.varchar))
        _ <- pgtestnullRepo.select.where(_.vector === row.vector).toChunk.map(x => assertJsonEquals(row.vector, x.head.vector))
//        _ <- pgtestnullRepo.select.where(_.xml === row.xml).toChunk.map(x => assertJsonEquals(row.xml, x.head.xml))
//        _ <- pgtestnullRepo.select.where(_.boxes === row.boxes).toChunk.map(x => assertJsonEquals(row.boxes, x.head.boxes))
//        _ <- pgtestnullRepo.select.where(_.bpchares === row.bpchares).toChunk.map(x => assertJsonEquals(row.bpchares, x.head.bpchares))
//        _ <- pgtestnullRepo.select.where(_.chares === row.chares).toChunk.map(x => assertJsonEquals(row.chares, x.head.chares))
//        _ <- pgtestnullRepo.select.where(_.circlees === row.circlees).toChunk.map(x => assertJsonEquals(row.circlees, x.head.circlees))
        _ <- pgtestnullRepo.select.where(_.datees === row.datees).toChunk.map(x => assertJsonEquals(row.datees, x.head.datees))
        _ <- pgtestnullRepo.select.where(_.float4es === row.float4es).toChunk.map(x => assertJsonEquals(row.float4es, x.head.float4es))
        _ <- pgtestnullRepo.select.where(_.float8es === row.float8es).toChunk.map(x => assertJsonEquals(row.float8es, x.head.float8es))
        _ <- pgtestnullRepo.select.where(_.inetes === row.inetes).toChunk.map(x => assertJsonEquals(row.inetes, x.head.inetes))
        _ <- pgtestnullRepo.select.where(_.int2es === row.int2es).toChunk.map(x => assertJsonEquals(row.int2es, x.head.int2es))
        _ <- pgtestnullRepo.select.where(_.int2vectores === row.int2vectores).toChunk.map(x => assertJsonEquals(row.int2vectores, x.head.int2vectores))
        _ <- pgtestnullRepo.select.where(_.int4es === row.int4es).toChunk.map(x => assertJsonEquals(row.int4es, x.head.int4es))
        _ <- pgtestnullRepo.select.where(_.int8es === row.int8es).toChunk.map(x => assertJsonEquals(row.int8es, x.head.int8es))
        _ <- pgtestnullRepo.select.where(_.intervales === row.intervales).toChunk.map(x => assertJsonEquals(row.intervales, x.head.intervales))
//        _ <- pgtestnullRepo.select.where(_.jsones === row.jsones).toChunk.map(x => assertJsonEquals(row.jsones, x.head.jsones))
        _ <- pgtestnullRepo.select.where(_.jsonbes === row.jsonbes).toChunk.map(x => assertJsonEquals(row.jsonbes, x.head.jsonbes))
//        _ <- pgtestnullRepo.select.where(_.linees === row.linees).toChunk.map(x => assertJsonEquals(row.linees, x.head.linees))
//        _ <- pgtestnullRepo.select.where(_.lseges === row.lseges).toChunk.map(x => assertJsonEquals(row.lseges, x.head.lseges))
        _ <- pgtestnullRepo.select.where(_.moneyes === row.moneyes).toChunk.map(x => assertJsonEquals(row.moneyes, x.head.moneyes))
        _ <- pgtestnullRepo.select.where(_.mydomaines === row.mydomaines).toChunk.map(x => assertJsonEquals(row.mydomaines, x.head.mydomaines))
        _ <- pgtestnullRepo.select.where(_.myenumes === row.myenumes).toChunk.map(x => assertJsonEquals(row.myenumes, x.head.myenumes))
//        _ <- pgtestnullRepo.select.where(_.namees === row.namees).toChunk.map(x => assertJsonEquals(row.namees, x.head.namees))
        _ <- pgtestnullRepo.select.where(_.numerices === row.numerices).toChunk.map(x => assertJsonEquals(row.numerices, x.head.numerices))
//        _ <- pgtestnullRepo.select.where(_.pathes === row.pathes).toChunk.map(x => assertJsonEquals(row.pathes, x.head.pathes))
//        _ <- pgtestnullRepo.select.where(_.pointes === row.pointes).toChunk.map(x => assertJsonEquals(row.pointes, x.head.pointes))
//        _ <- pgtestnullRepo.select.where(_.polygones === row.polygones).toChunk.map(x => assertJsonEquals(row.polygones, x.head.polygones))
        _ <- pgtestnullRepo.select.where(_.textes === row.textes).toChunk.map(x => assertJsonEquals(row.textes, x.head.textes))
        _ <- pgtestnullRepo.select.where(_.timees === row.timees).toChunk.map(x => assertJsonEquals(row.timees, x.head.timees))
        _ <- pgtestnullRepo.select.where(_.timestampes === row.timestampes).toChunk.map(x => assertJsonEquals(row.timestampes, x.head.timestampes))
        _ <- pgtestnullRepo.select.where(_.timestampzes === row.timestampzes).toChunk.map(x => assertJsonEquals(row.timestampzes, x.head.timestampzes))
        _ <- pgtestnullRepo.select.where(_.timezes === row.timezes).toChunk.map(x => assertJsonEquals(row.timezes, x.head.timezes))
        _ <- pgtestnullRepo.select.where(_.uuides === row.uuides).toChunk.map(x => assertJsonEquals(row.uuides, x.head.uuides))
//        _ <- pgtestnullRepo.select.where(_.varchares === row.varchares).toChunk.map(x => assertJsonEquals(row.varchares, x.head.varchares))
//        _ <- pgtestnullRepo.select.where(_.xmles === row.xmles).toChunk.map(x => assertJsonEquals(row.xmles, x.head.xmles))
      } yield ()
    }
  }
  ...
  test("can update pgtest with DSL") {
    withConnection {
      ...
      for {
        ...
        _ <- pgtestRepo.update.setValue(_.mydomaines)(row.mydomaines).where(_.uuid === row.uuid).execute
        ...
        _ <- pgtestRepo.update.setValue(_.numerices)(row.numerices).where(_.uuid === row.uuid).execute
