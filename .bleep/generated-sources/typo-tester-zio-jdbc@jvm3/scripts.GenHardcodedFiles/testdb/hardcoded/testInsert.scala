...
class testInsert(random: Random) {
  def compositepkPerson(name: Option[String] = if (random.nextBoolean()) None else Some(random.alphanumeric.take(20).mkString), one: Defaulted[Long] = Defaulted.UseDefault, two: Defaulted[Option[String]] = Defaulted.UseDefault): ZIO[ZConnection, Throwable, PersonRow] = (new PersonRepoImpl).insert(new PersonRowUnsaved(name = name, one = one, two = two))
