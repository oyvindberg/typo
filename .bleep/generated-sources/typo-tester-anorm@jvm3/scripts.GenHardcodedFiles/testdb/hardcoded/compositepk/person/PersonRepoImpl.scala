...
class PersonRepoImpl extends PersonRepo {
  ...
  def deleteById(compositeId: PersonId)(implicit c: Connection): Boolean = SQL"""delete from compositepk.person where "one" = ${ParameterValue(compositeId.one, null, ToStatement.longToStatement)} AND "two" = ${ParameterValue(compositeId.two, null, ToStatement.optionToStatement(ToStatement.stringToStatement, ParameterMetaData.StringParameterMetaData))}""".executeUpdate() > 0
  ...
  def insertStreaming(unsaved: Iterator[PersonRow], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY compositepk.person("one", "two", "name") FROM STDIN""", batchSize, unsaved)(PersonRow.text, c)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: Iterator[PersonRowUnsaved], batchSize: Int = 10000)(implicit c: Connection): Long = streamingInsert(s"""COPY compositepk.person("name", "one", "two") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(PersonRowUnsaved.text, c)
  def select: SelectBuilder[PersonFields, PersonRow] = SelectBuilderSql("compositepk.person", PersonFields.structure, PersonRow.rowParser)
  ...
  def update: UpdateBuilder[PersonFields, PersonRow] = UpdateBuilder("compositepk.person", PersonFields.structure, PersonRow.rowParser)
