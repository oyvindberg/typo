...
class testInsert(random: Random) {
  def compositepkPerson(name: Option[String] = if (random.nextBoolean()) None else Some(random.alphanumeric.take(20).mkString), one: Defaulted[Long] = Defaulted.UseDefault, two: Defaulted[Option[String]] = Defaulted.UseDefault): ConnectionIO[PersonRow] = PersonRepoImpl.insert(new PersonRowUnsaved(name = name, one = one, two = two))
  def myschemaFootballClub(id: FootballClubId, name: /* max 100 chars */ String = random.alphanumeric.take(20).mkString): ConnectionIO[FootballClubRow] = FootballClubRepoImpl.insert(new FootballClubRow(id = id, name = name))
