...
class MaritalStatusRepoImpl extends MaritalStatusRepo {
  ...
  def deleteById(id: MaritalStatusId): ConnectionIO[Boolean] = sql"""delete from myschema.marital_status where "id" = ${fromWrite(id)(Write.fromPut(MaritalStatusId.put))}""".update.run.map(_ > 0)
  def deleteByIds(ids: Array[MaritalStatusId]): ConnectionIO[Int] = sql"""delete from myschema.marital_status where "id" = ANY(${fromWrite(ids)(Write.fromPut(MaritalStatusId.arrayPut))})""".update.run
  ...
  def insertStreaming(unsaved: Stream[ConnectionIO, MaritalStatusRow], batchSize: Int = 10000): ConnectionIO[Long] = new FragmentOps(sql"""COPY myschema.marital_status("id") FROM STDIN""").copyIn(unsaved, batchSize)(using MaritalStatusRow.text)
  def select: SelectBuilder[MaritalStatusFields, MaritalStatusRow] = SelectBuilderSql("myschema.marital_status", MaritalStatusFields.structure, MaritalStatusRow.read)
  ...
  def selectById(id: MaritalStatusId): ConnectionIO[Option[MaritalStatusRow]] = sql"""select "id" from myschema.marital_status where "id" = ${fromWrite(id)(Write.fromPut(MaritalStatusId.put))}""".query(using MaritalStatusRow.read).option
  def selectByIds(ids: Array[MaritalStatusId]): Stream[ConnectionIO, MaritalStatusRow] = sql"""select "id" from myschema.marital_status where "id" = ANY(${fromWrite(ids)(Write.fromPut(MaritalStatusId.arrayPut))})""".query(using MaritalStatusRow.read).stream
  ...
  def update: UpdateBuilder[MaritalStatusFields, MaritalStatusRow] = UpdateBuilder("myschema.marital_status", MaritalStatusFields.structure, MaritalStatusRow.read)
    ...
    NonEmptyList.fromList(fieldValues) match {
      ...
      case Some(nonEmpty) =>
        val updates = fragments.set(
          nonEmpty.map {
            case MaritalStatusFieldValue.id(value) => fr""""id" = ${fromWrite(value)(Write.fromPut(MaritalStatusId.put))}::int8"""
          }
        )
        sql"""update myschema.marital_status
              $updates
              where "id" = ${fromWrite(id)(Write.fromPut(MaritalStatusId.put))}""".update.run.map(_ > 0)
    }
  }
