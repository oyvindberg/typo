...
trait PgStatSubscriptionRepoImpl extends PgStatSubscriptionRepo {
  ...
  override def selectByFieldValues(fieldValues: List[PgStatSubscriptionFieldValue[_]])(implicit c: Connection): List[PgStatSubscriptionRow] = {
    fieldValues match {
      ...
      case nonEmpty =>
        ...
        SQL"""select * from pg_catalog.pg_stat_subscription where ${namedParams.map(x => s"${x.name} = {${x.name}}").mkString(" AND ")}"""
          .on(namedParams: _*)
