...
trait PgStatioAllIndexesRepoImpl extends PgStatioAllIndexesRepo {
  ...
  override def selectByFieldValues(fieldValues: List[PgStatioAllIndexesFieldValue[_]])(implicit c: Connection): List[PgStatioAllIndexesRow] = {
    fieldValues match {
      ...
      case nonEmpty =>
        ...
        SQL"""select * from pg_catalog.pg_statio_all_indexes where ${namedParams.map(x => s"${x.name} = {${x.name}}").mkString(" AND ")}"""
          .on(namedParams: _*)
