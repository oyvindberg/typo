...
trait ElementTypesRepoImpl extends ElementTypesRepo {
  ...
  override def selectByFieldValues(fieldValues: List[ElementTypesFieldValue[_]])(implicit c: Connection): List[ElementTypesRow] = {
    fieldValues match {
      ...
      case nonEmpty =>
        ...
        SQL"""select * from information_schema.element_types where ${namedParams.map(x => s"${x.name} = {${x.name}}").mkString(" AND ")}"""
          .on(namedParams: _*)
