...
class PersonRepoImpl extends PersonRepo {
  ...
  def deleteById(id: PersonId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from myschema.person where "id" = ${Segment.paramSegment(id)(PersonId.setter)}""".delete.map(_ > 0)
  def deleteByIds(ids: Array[PersonId]): ZIO[ZConnection, Throwable, Long] = sql"""delete from myschema.person where "id" = ANY(${Segment.paramSegment(ids)(PersonId.arraySetter)})""".delete
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, PersonRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY myschema.person("id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "sector", "favorite_number") FROM STDIN""", batchSize, unsaved)(PersonRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, PersonRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY myschema.person("favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "work_email", "id", "marital_status_id", "sector", "favorite_number") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(PersonRowUnsaved.text)
  def select: SelectBuilder[PersonFields, PersonRow] = SelectBuilderSql("myschema.person", PersonFields.structure, PersonRow.jdbcDecoder)
  ...
  def selectById(id: PersonId): ZIO[ZConnection, Throwable, Option[PersonRow]] = sql"""select "id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "sector", "favorite_number" from myschema.person where "id" = ${Segment.paramSegment(id)(PersonId.setter)}""".query(using PersonRow.jdbcDecoder).selectOne
  def selectByIds(ids: Array[PersonId]): ZStream[ZConnection, Throwable, PersonRow] = sql"""select "id", "favourite_football_club_id", "name", "nick_name", "blog_url", "email", "phone", "likes_pizza", "marital_status_id", "work_email", "sector", "favorite_number" from myschema.person where "id" = ANY(${Segment.paramSegment(ids)(PersonId.arraySetter)})""".query(using PersonRow.jdbcDecoder).selectStream()
  ...
  def update: UpdateBuilder[PersonFields, PersonRow] = UpdateBuilder("myschema.person", PersonFields.structure, PersonRow.jdbcDecoder)
  ...
  def updateFieldValues(id: PersonId, fieldValues: List[PersonFieldValue[?]]): ZIO[ZConnection, Throwable, Boolean] = {
    NonEmptyChunk.fromIterableOption(fieldValues) match {
      ...
      case Some(nonEmpty) =>
        val updates = nonEmpty.map { case PersonFieldValue.id(value) => sql""""id" = ${Segment.paramSegment(value)(PersonId.setter)}::int8"""
                                     case PersonFieldValue.favouriteFootballClubId(value) => sql""""favourite_football_club_id" = ${Segment.paramSegment(value)(FootballClubId.setter)}"""
                                     case PersonFieldValue.name(value) => sql""""name" = ${Segment.paramSegment(value)(Setter.stringSetter)}"""
                                     case PersonFieldValue.nickName(value) => sql""""nick_name" = ${Segment.paramSegment(value)(Setter.optionParamSetter(Setter.stringSetter))}"""
                                     case PersonFieldValue.blogUrl(value) => sql""""blog_url" = ${Segment.paramSegment(value)(Setter.optionParamSetter(Setter.stringSetter))}"""
