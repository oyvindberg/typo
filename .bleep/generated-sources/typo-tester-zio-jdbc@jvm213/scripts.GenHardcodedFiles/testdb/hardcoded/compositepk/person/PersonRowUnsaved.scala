...
case class PersonRowUnsaved(name: Option[String], /** Default: auto-increment */
                            one: Defaulted[Long] = Defaulted.UseDefault(), /** Default: auto-increment */
                            two: Defaulted[Option[String]] = Defaulted.UseDefault()) {
  def toRow(oneDefault: => Long, twoDefault: => Option[String]): PersonRow = new PersonRow(one = one.getOrElse(oneDefault), two = two.getOrElse(twoDefault), name = name)
}
