...
class PersonRepoImpl extends PersonRepo {
  def delete: DeleteBuilder[PersonFields, PersonRow] = DeleteBuilder("compositepk.person", PersonFields.structure)
  def deleteById(compositeId: PersonId): ZIO[ZConnection, Throwable, Boolean] = sql"""delete from compositepk.person where "one" = ${Segment.paramSegment(compositeId.one)(Setter.longSetter)} AND "two" = ${Segment.paramSegment(compositeId.two)(Setter.optionParamSetter(Setter.stringSetter))}""".delete.map(_ > 0)
  ...
  def insertStreaming(unsaved: ZStream[ZConnection, Throwable, PersonRow], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY compositepk.person("one", "two", "name") FROM STDIN""", batchSize, unsaved)(PersonRow.text)
  /** NOTE: this functionality requires PostgreSQL 16 or later! */
  def insertUnsavedStreaming(unsaved: ZStream[ZConnection, Throwable, PersonRowUnsaved], batchSize: Int = 10000): ZIO[ZConnection, Throwable, Long] = streamingInsert(s"""COPY compositepk.person("name", "one", "two") FROM STDIN (DEFAULT '__DEFAULT_VALUE__')""", batchSize, unsaved)(PersonRowUnsaved.text)
  def select: SelectBuilder[PersonFields, PersonRow] = SelectBuilderSql("compositepk.person", PersonFields.structure, PersonRow.jdbcDecoder)
  def selectAll: ZStream[ZConnection, Throwable, PersonRow] = sql"""select "one", "two", "name" from compositepk.person""".query(using PersonRow.jdbcDecoder).selectStream()
  ...
  def selectById(compositeId: PersonId): ZIO[ZConnection, Throwable, Option[PersonRow]] = sql"""select "one", "two", "name" from compositepk.person where "one" = ${Segment.paramSegment(compositeId.one)(Setter.longSetter)} AND "two" = ${Segment.paramSegment(compositeId.two)(Setter.optionParamSetter(Setter.stringSetter))}""".query(using PersonRow.jdbcDecoder).selectOne
  def update: UpdateBuilder[PersonFields, PersonRow] = UpdateBuilder("compositepk.person", PersonFields.structure, PersonRow.jdbcDecoder)
  ...
  def updateFieldValues(compositeId: PersonId, fieldValues: List[PersonFieldValue[?]]): ZIO[ZConnection, Throwable, Boolean] = {
    NonEmptyChunk.fromIterableOption(fieldValues) match {
      ...
      case Some(nonEmpty) =>
        val updates = nonEmpty.map { case PersonFieldValue.one(value) => sql""""one" = ${Segment.paramSegment(value)(Setter.longSetter)}::int8"""
                                     case PersonFieldValue.two(value) => sql""""two" = ${Segment.paramSegment(value)(Setter.optionParamSetter(Setter.stringSetter))}"""
                                     case PersonFieldValue.name(value) => sql""""name" = ${Segment.paramSegment(value)(Setter.optionParamSetter(Setter.stringSetter))}""" }.mkFragment(SqlFragment(", "))
        sql"""update compositepk.person
              set $updates
